...
/// `Scratch` directory.
const FORMATTED_NAME: &str = "formatted.java";
const ORIGINAL_NAME: &str = "original.java";
const SELF_TEST_NAME: &str = "selftest.java";
...
impl Tools for SystemTools {
    ...
    fn run(&self, program: &str, args: &[&str], file: &Path) -> io::Result<Outcome> {
        let output = Command::new(program)
            ...
            .stdin(Stdio::null())
            .output()?;
        // The exit status is the whole verdict; see the module docs on why
        // stderr cannot be it.
...
fn check_with(tools: &dyn Tools, original: &str, code: &str) -> Result<()> {
    let program = tools.locate(&JAVAC_NAMES).ok_or_else(|| {
        Error::Verification(
            "external verification needs `javac` on PATH: it was not found".to_string(),
        )
    })?;
    let scratch = Scratch::new()?;

    // The gate is tested before it is trusted: an `-XD` key this JDK does not
    // recognise is ignored rather than rejected, which would turn the gate
    // into a full compile without a word. See the module docs.
    let fixture = scratch.write(SELF_TEST_NAME, SELF_TEST_SOURCE)?;
    if let Outcome::Fail(message) = tools.run(&program, &JAVAC_ARGS, &fixture)? {
        return Err(Error::Verification(format!(
            "external verification cannot trust `{program} -XDshould-stop.ifNoError=PARSE`: \
             ...
             would be a whole-program compile: {message}"
        )));
    }

    let before = scratch.write(ORIGINAL_NAME, original)?;
    if let Outcome::Fail(_) = tools.run(&program, &JAVAC_ARGS, &before)? {
    ...
    match tools.run(&program, &JAVAC_ARGS, &after)? {
        Outcome::Pass => Ok(()),
        Outcome::Fail(message) => Err(Error::Verification(format!(
            "external check failed ({program} -XDshould-stop.ifNoError=PARSE): {message}"
        ))),
    }
}
...
mod tests {
    ...
    #[test]
    fn no_javac_on_path_is_an_error_naming_the_tool() {
    ...
    #[test]
    fn javac_sees_the_self_test_then_the_original_then_the_output() {
        let tools = FakeTools::new(&["javac"], vec![pass(), pass(), pass()]);
        check_with(&tools, VALID, FORMATTED).unwrap();
        let calls = tools.calls();
        assert_eq!(calls.len(), 3);
        for call in &calls {
            assert_eq!(call.program, "javac");
            assert_eq!(call.args, ["-XDshould-stop.ifNoError=PARSE"]);
            // A `.java` file in a private directory, never the user's own path.
            assert_eq!(call.file.extension().unwrap(), "java");
            ...
            // All three share one scratch directory.
            assert_eq!(call.file.parent(), calls[0].file.parent());
        }
        assert_eq!(calls[0].contents, SELF_TEST_SOURCE);
        assert_eq!(calls[1].contents, VALID);
        assert_eq!(calls[2].contents, FORMATTED);
        assert_ne!(calls[0].file, calls[1].file);
        assert_ne!(calls[1].file, calls[2].file);
    }

    #[test]
    fn a_self_test_the_jdk_fails_stops_the_gate_naming_the_option() {
        // The failure this exists for: a JDK that renamed the `-XD` key would
        // ignore it silently and compile the whole program, so the fixture --
        // valid Java that only a full compile can reject -- comes back
        // rejected. That must fail loudly rather than blame the user's file.
        let tools = FakeTools::new(
            &["javac"],
            vec![fail(
                "selftest.java:1: error: package com.example does not exist",
            )],
        );
        let err = check_with(&tools, VALID, FORMATTED).unwrap_err();
        assert!(matches!(err, Error::Verification(_)), "{err}");
        let message = err.to_string();
        assert!(
            message.contains("-XDshould-stop.ifNoError=PARSE"),
            "{message}"
        );
    ...
    fn output_that_fails_the_external_check_is_rejected() {
        let tools = FakeTools::new(
            ...
            vec![
                pass(),
                pass(),
                fail("formatted.java:2: error: reached end of file while parsing"),
            ],
        );
        ...
        let message = err.to_string();
        assert!(
            message.contains("javac -XDshould-stop.ifNoError=PARSE"),
            "{message}"
        );
        assert!(
    ...
    fn an_original_that_already_fails_is_not_blamed_on_the_formatter() {
        ...
        let tools = FakeTools::new(
            ...
            vec![
                pass(),
                fail("original.java:1: error: integer number too large"),
            ],
        );
        check_with(
            &tools,
    ...
    fn a_checker_that_cannot_be_spawned_is_an_error() {
        let tools = FakeTools::new(&["javac"], vec![Err(io::Error::other("spawn failed"))]);
        let err = check_with(&tools, VALID, FORMATTED).unwrap_err();
    ...
    #[test]
    fn the_probe_is_the_version_flag() {
        // Pinned because both obvious alternatives are traps on javac 21:
        // `javac -h` exits 2 (it is `--help-extra`'s neighbour only in other
        // toolchains; here `-h <dir>` wants a native-header directory), and a
        // bare `javac` exits 2 with `no source files`. Either would make a
        // perfectly good JDK look absent.
        assert_eq!(JAVAC_PROBE_ARGS, ["-version"]);
        assert!(!JAVAC_PROBE_ARGS.is_empty());
    }
    ...
    fn system_tools_locate_walks_the_candidates_in_order() {
        ...
        assert_eq!(
            SystemTools.locate(&["tokenpress-no-such-tool", "javac"]),
        ...
        assert_eq!(SystemTools.locate(&JAVAC_NAMES), Some("javac".to_string()));
    }

    #[test]
    fn system_tools_run_reports_javacs_verdict() {
        let scratch = Scratch::new().unwrap();
        let good = scratch.write("good.java", VALID).unwrap();
        let bad = scratch.write("bad.java", BROKEN).unwrap();
        assert_eq!(
            verdict(SystemTools.run("javac", &JAVAC_ARGS, &good).unwrap()),
            ""
        );
        let message = verdict(SystemTools.run("javac", &JAVAC_ARGS, &bad).unwrap());
        assert!(
            message.contains("reached end of file while parsing"),
            "{message}"
        );
    ...
    #[test]
    fn a_public_class_need_not_be_named_after_its_file() {
        // JLS 7.6 is a *semantic* rule and this gate stops before the phase
        // that applies it, which is what lets the fixed `original.java` /
        // `formatted.java` names work for every input. Asserted against the
        // real compiler, because the whole scheme rests on it.
        let scratch = Scratch::new().unwrap();
        let mismatched = scratch.write(FORMATTED_NAME, FORMATTED).unwrap();
        assert_eq!(
            verdict(SystemTools.run("javac", &JAVAC_ARGS, &mismatched).unwrap()),
            ""
        );
    }

    #[test]
    fn the_self_test_fixture_passes_the_gate_and_fails_a_full_compile() {
        // Both halves matter. The first is the row the design turns on: a file
        // whose imports cannot be resolved is still a clean *parse*. The
        // second is what makes the fixture discriminating at all — without the
        // flag the very same file is rejected, so a JDK that quietly ignored
        // the flag could not pass this check.
        let scratch = Scratch::new().unwrap();
        let fixture = scratch.write(SELF_TEST_NAME, SELF_TEST_SOURCE).unwrap();
        assert_eq!(
            verdict(SystemTools.run("javac", &JAVAC_ARGS, &fixture).unwrap()),
            ""
        );
        let full = verdict(SystemTools.run("javac", &[], &fixture).unwrap());
        assert!(full.contains("does not exist"), "{full}");
    }

    #[test]
    fn what_javac_prints_to_stderr_is_not_the_verdict() {
        // The JVM launcher announces `JAVA_TOOL_OPTIONS` on stderr on *every*
        // invocation, successful ones included. The condition is created here
        // deliberately rather than waited for: a stderr-based gate would
        // reject this file, and would reject every file on any machine that
        // sets the variable.
        let scratch = Scratch::new().unwrap();
        let good = scratch.write("good.java", VALID).unwrap();
        let noisy = Command::new("javac")
            .args(JAVAC_ARGS)
            .arg(&good)
            .env("JAVA_TOOL_OPTIONS", "-Dtokenpress.stderr.probe=1")
            .stdin(Stdio::null())
            .output()
            .unwrap();
        assert!(noisy.status.success());
        assert!(
            !noisy.stderr.is_empty(),
            "the launcher was expected to announce JAVA_TOOL_OPTIONS on stderr"
        );
        // ... and the exit status alone is what the checker reads, so the same
        // file is a pass.
        assert_eq!(
            verdict(SystemTools.run("javac", &JAVAC_ARGS, &good).unwrap()),
            ""
    ...
    #[test]
    fn javac_accepts_output_it_can_still_parse() {
        check_with(&SystemTools, VALID, FORMATTED).unwrap();
    }

    #[test]
    fn javac_accepts_output_whose_imports_cannot_be_resolved() {
        // The row the whole design turns on, run through the orchestration
        // rather than through `SystemTools` alone: a file naming types no
        // classpath provides is exactly what a real project's file looks like
    ...
    #[test]
    fn javac_rejects_output_it_cannot_parse() {
        let err = check_with(&SystemTools, VALID, BROKEN).unwrap_err();
        let message = err.to_string();
        assert!(
            message.contains("external check failed (javac -XDshould-stop.ifNoError=PARSE)"),
            "{message}"
        );
        assert!(
            message.contains("reached end of file while parsing"),
    ...
    #[test]
    fn input_javac_already_rejects_does_not_fail_the_run() {
        // `long x = 99999999999;` — a decimal literal with no `L` suffix. The
        // grammar parses it happily; javac calls it `integer number too
        // large`, at every release, so this stays a disagreement rather than
        // ageing into agreement the way a preview-syntax fixture would. The
        // formatted output would be refused for exactly the same reason, so
        // the policy is what keeps it from being reported as a TokenPress
        // failure.
        check_with(
            &SystemTools,
            "class A {\n    long x = 99999999999;\n}\n",
    ...
    fn scratch_directories_are_unique_and_removed_on_drop() {
        ...
        let path = first.write("A.java", VALID).unwrap();
