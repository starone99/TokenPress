...
/// nothing.
pub fn has_escape_hazard(tree:&Tree,source:&[u8])->bool{node_has_escape_hazard(tree.root_node(),source)}
...
pub type JavaCommentPolicy=CommentPolicy<fn(&[u8])->bool,fn(&Tree,&[u8])->Range<usize>,fn(&Tree,&[u8])->bool>;
...
pub fn comment_policy()->JavaCommentPolicy{CommentPolicy::new(is_semantic_comment,no_prologue,has_escape_hazard)}
...
fn node_has_escape_hazard(node:Node,source:&[u8])->bool{if COMMENT_KINDS.contains(&node.kind()){return holds_hazardous_escape(&source[node.byte_range()]);}let mut cursor=node.walk();let mut children=node.children(&mut cursor);children.any(|child|node_has_escape_hazard(child,source))}
...
/// here.
fn holds_hazardous_escape(bytes:&[u8])->bool{let mut index=0;while index<bytes.len(){if bytes[index]!=b'\\'{index+=1;continue;}let run=bytes[index..].iter().take_while(|byte|**byte==b'\\').count();if run%2==1&&escape_value(&bytes[index+run..]).is_some_and(|value|HAZARDOUS_ESCAPE_VALUES.contains(&value)){return true;}index+=run;}false}
...
fn escape_value(after_backslash:&[u8])->Option<u32>{let markers=after_backslash.iter().take_while(|byte|**byte==b'u').count();if markers==0{return None;}let digits=after_backslash.get(markers..markers+4)?;let mut value=0;for digit in digits{value=value*16+char::from(*digit).to_digit(16)?;}Some(value)}#[cfg(test)]mod tests{use super::*;use crate::config::java_config;use tokenpress_treesitter::emit::strip_comments_source;use tokenpress_treesitter::parser::parse;
...
/// no column-0 rule.
fn stripped(source:&[u8])->Vec<u8>{strip_comments_source(&java_config(),source,&comment_policy()).unwrap()}
...
fn kept(source:&[u8])->Vec<u8>{let policy=CommentPolicy::new(keeps_every_comment,no_prologue,has_escape_hazard);strip_comments_source(&java_config(),source,&policy).unwrap()}
/// The stripping emitter with the bail-out disarmed: what the output would
/// be if [`has_escape_hazard`] did not exist.
fn stripped_without_the_bail_out(source:&[u8])->Vec<u8>{let policy=CommentPolicy::new(is_semantic_comment,no_prologue,never_bails_out);strip_comments_source(&java_config(),source,&policy).unwrap()}fn keeps_every_comment(_bytes:&[u8])->bool{true}fn never_bails_out(_tree:&Tree,_source:&[u8])->bool{false}
/// Whether the bail-out fires for a source the engine accepts.
fn bails_out(source:&[u8])->bool{let config=java_config();let tree=parse(&config,source).unwrap();has_escape_hazard(&tree,source)}#[test]fn the_escape_reproduction_is_left_byte_identical(){let source=b"class A {\nint x = 1; // c \\u000A int y = 2;\nint z = 3;\n}\n";assert!(bails_out(source));assert_eq!(stripped(source),source);assert_eq!(stripped_without_the_bail_out(source),&b"class A {\nint x = 1;\nint z = 3;\n}\n"[..]);}#[test]fn a_file_that_trips_the_bail_out_is_byte_identical_at_both_settings(){let source=b"class A {\nint x = 1; // c \\u000A int y = 2;\nint z = 3;\n}\n";assert_eq!(kept(source),source);assert_eq!(stripped(source),source);}#[test]fn the_narrowness_table_holds(){for(escape,expected)in[("\\u2192",false),("\\u0020",false),("\\u0000",false),("\\u2028",false),("\\u2029",false),("\\u000A",true),("\\u000D",true),("\\u002A",true),("\\u002F",true),("\\u000a",true),("\\uu000A",true),("\\uuuu000A",true),("\\\\u000A",false),]{let source=format!("class A {{ // note {escape}\n}}\n");assert_eq!(bails_out(source.as_bytes()),expected,"{escape}");}}#[test]fn a_block_comment_is_scanned_too(){assert!(bails_out(b"class A { /* c \\u002A/ int y = 2; */ }\n"));assert!(bails_out(b"class A { /* c *\\u002F int y = 2; */ }\n"));assert!(!bails_out(b"class A { /* c \\u0041 */ }\n"));}#[test]fn backslash_parity_counts_the_whole_run(){assert!(bails_out(b"class A { // c \\u000A\n}\n"));assert!(!bails_out(b"class A { // c \\\\u000A\n}\n"));assert!(bails_out(b"class A { // c \\\\\\u000A\n}\n"));assert!(!bails_out(b"class A { // c \\\\\\\\u000A\n}\n"));}#[test]fn a_malformed_escape_is_not_a_hazard(){assert!(!bails_out(b"class A { // a windows path C:\\temp\n}\n"));assert!(!bails_out(b"class A { // c \\uZZZZ\n}\n"));assert!(!bails_out(b"class A { // c \\u00\n}\n"));assert!(!bails_out(b"class A { // c \\\n}\n"));}#[test]fn an_escape_outside_a_comment_does_not_bail_out(){let source=b"class A {\nString s = \"\\u002A\"; // note\n}\n";assert!(!bails_out(source));assert_eq!(stripped(source),&b"class A {\nString s = \"\\u002A\";\n}\n"[..]);}#[test]fn the_bail_out_reaches_a_comment_nested_in_a_method_body(){assert!(bails_out(b"class A {\nvoid m() {\nint x = 1; // c \\u000A int y = 2;\n}\n}\n"));}#[test]fn a_file_with_no_escape_at_all_does_not_bail_out(){assert!(!bails_out(b"class A {\n// plain\n/* plain */\n}\n"));assert!(!bails_out(b""));}#[test]fn no_java_comment_is_semantic(){for comment in[&b"// an ordinary note"[..],b"/** Javadoc. */",b"/* a block comment */",b"//",]{let text=String::from_utf8_lossy(comment);assert!(!is_semantic_comment(comment),"{text}");}}#[test]fn third_party_tool_comments_are_deleted_like_any_other(){for comment in[&b"// NOSONAR"[..],b"// NOPMD",b"//CHECKSTYLE:OFF",b"// @formatter:off",b"//$NON-NLS-1$",]{let text=String::from_utf8_lossy(comment);assert!(!is_semantic_comment(comment),"{text}");}let source=b"class A {\nint x = 1; // NOSONAR\n}\n";assert_eq!(stripped(source),&b"class A {\nint x = 1;\n}\n"[..]);}#[test]fn the_prologue_is_always_empty(){let config=java_config();for source in[&b""[..],b"class A {}\n",b"/** Package docs. */\n@Deprecated\npackage a;\n",]{let tree=parse(&config,source).unwrap();assert_eq!(no_prologue(&tree,source),0..0);}}#[test]fn a_package_info_keeps_its_annotation_adjacent_to_the_package_clause(){assert_eq!(stripped(b"/** Package docs. */\n@Deprecated\npackage a;\n"),&b"@Deprecated\npackage a;\n"[..]);}#[test]fn the_policy_deletes_every_comment_and_keeps_the_code(){let source=b"/** Doc. */\nclass A {\n// note\nint x = 1; /* trailing */\nString s = \"// not a comment\";\n}\n";assert_eq!(stripped(source),&b"class A {\nint x = 1;\nString s = \"// not a comment\";\n}\n"[..]);assert_eq!(kept(source),&b"/** Doc. */\nclass A {\n// note\nint x = 1; /* trailing */\nString s = \"// not a comment\";\n}\n"[..]);}#[test]fn the_scanned_kinds_are_the_configured_comment_kinds(){assert_eq!(java_config().comment_kinds(),COMMENT_KINDS);}}