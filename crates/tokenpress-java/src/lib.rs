...
fn comment_policy(strip_comments: bool) -> policy::JavaCommentPolicy {
    let keep_comment = if strip_comments {
        policy::is_semantic_comment
    } else {
        keep_every_comment as fn(&[u8]) -> bool
    };
    CommentPolicy::new(keep_comment, policy::no_prologue, policy::has_escape_hazard)
}
...
impl Formatter for JavaFormatter {
    ...
    fn format(&self, _path: &Path, source: &str, options: &FormatOptions) -> Result<FormatResult> {
        ...
        // The plain emitter, not `strip_comments_pinned`: nothing in Java
        // changes meaning by arriving at column 0. The plan is built here
        // rather than through the engine's one-call `strip_comments_source`
        // so the tree is parsed once and not again.
...
mod tests {
    ...
    fn fmt(source: &str) -> String {
        fmt_with(source, JavaOptions::default())
    }
    ...
    fn fmt_with(source: &str, options: JavaOptions) -> String {
        JavaFormatter::new(options)
    ...
    fn supports_the_java_paths() {
        ...
        for name in ["a.go", "a.rb", "A.JAVA", "A.class", "A.jsh", "java"] {
            assert!(!f.supports(Path::new(name)), "{name} should be rejected");
        }
    }

    #[test]
    fn jv01_minimizes_whitespace() {
        let source = "/**\n * Doc.\n */\npublic class A {\n    /** The field. */\n    private int x = 1;\n\n    public int x() {\n        return x;\n    }\n}\n";
        assert_eq!(
            fmt(source),
            "/**\n * Doc.\n */\npublic class A {\n/** The field. */\nprivate int x = 1;\npublic int x() {\nreturn x;\n}\n}\n"
    ...
    #[test]
    fn jvo1_defaults_to_keeping_comments() {
        assert!(!JavaOptions::default().strip_comments);
    }

    #[test]
    fn a_text_block_survives_byte_for_byte_at_both_settings() {
        let source = "class A {\n    // note\n    String s = \"\"\"\n        one\n          two\n        \"\"\";\n}\n";
        let interior = "\"\"\"\n        one\n          two\n        \"\"\"";
        let kept = fmt(source);
    ...
    #[test]
    fn jv01_collapses_alignment_and_blank_lines() {
    ...
    #[test]
    fn jv01_drops_the_leading_run_of_the_file() {
        assert_eq!(fmt("\n\n   class  A  {}\n"), "class A {}\n");
    }

    #[test]
    fn jvo1_keeps_every_comment_by_default() {
        let source = "class A {\n    // leading\n    int x = 1; // trailing\n    /* inline */ int y = 2;\n}\n";
        assert_eq!(
            fmt(source),
            "class A {\n// leading\nint x = 1; // trailing\n/* inline */ int y = 2;\n}\n"
    ...
    #[test]
    fn jvo1_strips_comments_on_request() {
    ...
    fn jvo1_deletes_javadoc_when_stripping() {
        ...
        // API documentation goes with every other comment. Nothing is dropped
        // at the default settings, which the second half asserts.
        let source = "/**\n * The class.\n */\npublic class A {\n    /** The method. */\n    public void m() {}\n}\n";
        assert_eq!(
            stripped(source),
            "public class A {\npublic void m() {}\n}\n"
    ...
    fn an_indented_comment_is_emitted_at_column_0() {
        // The simplification over Go, pinned as behaviour rather than as
        // prose. Go has to keep `//go:generate` off column 0, because moving
        // it there makes `go generate` run it; Java has no comment syntax
        // that is a directive only at column 0, so the plain emitter is used
        // and an indented comment simply loses its indentation like anything
        // else. Both settings run that one path.
        let source = "class A {\n    void m() {\n        // note\n        int x = 1;\n    }\n}\n";
        assert_eq!(
            fmt(source),
            "class A {\nvoid m() {\n// note\nint x = 1;\n}\n}\n"
    ...
    fn an_escape_hazard_file_is_left_byte_identical_at_both_settings() {
        ...
        // `line_comment` node here while javac compiles the field. The
        // bail-out is one of the two callbacks both settings share, and the
        // plan it yields protects every byte. See `policy::has_escape_hazard`
        // for the end-to-end reproduction.
        let source = "class A {\nint x = 1; // c \\u000A int y = 2;\nint z = 3;\n}\n";
    ...
    #[test]
    fn an_escape_hazard_file_reports_no_savings() {
    ...
    fn the_path_is_not_read() {
        // tree-sitter-java has no dialect, filepath or version selector, so a
        // supported path is the caller's filter and nothing more — which is
        // also why `module-info.java` needs no special case.
        let r = JavaFormatter::default()
            .format(
                Path::new("notes.txt"),
                "class  A  {}\n",
    ...
    fn formatting_is_idempotent() {
        let sources = [
            // A text block, whose interior is a protected span.
            "class A {\n    String s = \"\"\"\n        one\n          two\n        \"\"\";\n}\n",
            // A Javadoc'd class.
            "/**\n * Doc.\n */\npublic class A {\n    /** The field. */\n    private int x = 1;\n}\n",
            // A record.
            ...
            // ...and the escape-hazard file the bail-out reproduces verbatim.
            "class A {\nint x = 1; // c \\u000A int y = 2;\nint z = 3;\n}\n",
        ];
