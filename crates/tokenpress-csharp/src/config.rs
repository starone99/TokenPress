...
pub fn csharp_config() -> LanguageConfig {
    ...
    // is unwrapped here, exactly as `tokenpress-go` and `tokenpress-java` do
    // and as the engine unwraps `set_language` for its sibling impossibility
    // (the ABI window). What keeps that honest is the same thing: tests below
    // that assert each kind against the grammar, so a grammar bump that
    // renames one fails `cargo test` rather than a user's run.
    .expect("every configured kind is a named node kind of the pinned tree-sitter-c-sharp grammar")
}
...
mod tests {
    ...
    fn sexp(source: &[u8]) -> String {
        parse(&csharp_config(), source)
            .unwrap()
            .root_node()
            .to_sexp()
    }
    ...
    fn the_grammar_abi_is_inside_the_runtime_window() {
        let abi = csharp().abi_version();
        assert!(
            (MIN_COMPATIBLE_LANGUAGE_VERSION..=LANGUAGE_VERSION).contains(&abi),
            "tree-sitter-c-sharp reports ABI {abi}, outside [{MIN_COMPATIBLE_LANGUAGE_VERSION}, {LANGUAGE_VERSION}]"
        );
        assert_eq!(csharp_config().language().abi_version(), abi);
    }

    #[test]
    fn the_configuration_is_the_csharp_one() {
    ...
    fn every_configured_kind_is_validated_against_the_grammar() {
        ...
        for &kind in config.comment_kinds() {
            assert!(
                LanguageConfig::new(csharp(), vec![kind], vec![], true).is_ok(),
                "{kind} should be a named kind of the grammar"
            );
        }
        for &kind in config.protected_kinds() {
            assert!(
                LanguageConfig::new(csharp(), vec![], vec![kind], true).is_ok(),
                "{kind} should be a named kind of the grammar"
            );
        }
        let err =
            LanguageConfig::new(csharp(), vec![], vec!["not_a_csharp_kind"], true).unwrap_err();
        assert_eq!(
            err.to_string(),
            "node kind not in this grammar: not_a_csharp_kind"
        );
    }

    #[test]
    fn javas_two_comment_kinds_are_refused_while_the_single_kind_is_accepted() {
        for kind in ["line_comment", "block_comment"] {
            let err = LanguageConfig::new(csharp(), vec![kind], vec![], true).unwrap_err();
            assert_eq!(
                err.to_string(),
                format!("node kind not in this grammar: {kind}")
            );
        }
        assert!(LanguageConfig::new(csharp(), vec!["comment"], vec![], true).is_ok());
    }

    #[test]
    fn xml_doc_line_and_block_comments_are_all_the_one_comment_kind() {
        let sexp = sexp(b"/// <summary>Doc.</summary>\n// line\n/* block */\nclass A {}\n");
        assert_eq!(sexp.matches("(comment)").count(), 3, "{sexp}");
        for absent in ["documentation_comment", "line_comment", "block_comment"] {
            assert!(!sexp.contains(absent), "{absent} unexpectedly in {sexp}");
        }
    }

    #[test]
    fn each_string_form_has_the_kind_the_configuration_names() {
        for (source, kind) in [
            (&b"class A { string s = \"hi\"; }"[..], "string_literal"),
            (
                b"class A { string s = @\"hi\"; }",
                "verbatim_string_literal",
            ),
            (
                b"class A { string s = \"\"\"hi\"\"\"; }",
            ...
            (
                b"class A { string s = $\"a{1}b\"; }",
                "interpolated_string_expression",
            ),
            (b"class A { char c = 'y'; }", "character_literal"),
        ] {
    ...
    #[test]
    fn a_utf8_literal_is_its_unsuffixed_kind_with_the_suffix_inside_the_span() {
        for (source, kind) in [
            (
                &b"class A { System.ReadOnlySpan<byte> s => \"hi\"u8; }"[..],
                "string_literal",
            ),
            (
                b"class A { System.ReadOnlySpan<byte> s => @\"hi\"u8; }",
        ...
        for absent in [
            "utf8_string_literal",
            "utf8_verbatim_string_literal",
            "utf8_raw_string_literal",
        ] {
            let err = LanguageConfig::new(csharp(), vec![], vec![absent], true).unwrap_err();
            assert_eq!(
                err.to_string(),
                format!("node kind not in this grammar: {absent}")
            );
        }
        // The `u8` suffix is inside the literal's span, so protecting the
        // literal protects the suffix: `"hi"u8` and `hi` are different
        // expressions and the emitter must not be able to separate them.
        let source = b"class A { System.ReadOnlySpan<byte> s => \"hi\"u8; }";
        assert_eq!(
            emit::minimize_source(&csharp_config(), source).unwrap(),
            b"class A { System.ReadOnlySpan<byte> s => \"hi\"u8; }".to_vec()
        );
    }

    #[test]
    fn all_three_interpolated_quotings_are_one_kind() {
        for source in [
            &b"class A { string s = $\"a{1}b\"; }"[..],
            b"class A { string s = $@\"a{1}b\"; }",
            b"class A { string s = $\"\"\"a{1}b\"\"\"; }",
            ...
            let sexp = sexp(source);
            assert!(sexp.contains("interpolated_string_expression"), "{sexp}");
            for absent in ["verbatim_string_literal", "raw_string_literal"] {
                assert!(!sexp.contains(absent), "{absent} unexpectedly in {sexp}");
            }
        }
    }
    ...
    #[test]
    fn a_raw_string_interior_survives_byte_for_byte() {
        let source =
            b"class A {\n    string s = \"\"\"\n        one\n          two\n        \"\"\";\n}\n";
        let out = emit::minimize_source(&csharp_config(), source).unwrap();
    ...
    #[test]
    fn both_newline_hazards_are_real_and_the_flag_is_what_defends_them() {
        // The measurement behind `newline_sensitive = true`, reduced to the
        // two shapes that caused it. Under `false` a whitespace run without a
        // newline *and* one with a newline both become a single space, so the
        // emitter joins lines — and C# has two independent ways for that to
        // change the program, where Java had one.
        let joining = LanguageConfig::new(
            csharp_config().language().clone(),
            vec!["comment"],
            vec![],
            false,
        )
        .unwrap();
        for source in [
            // (1) The `//` comment swallows whatever is joined onto its line.
            &b"class A {\n    // note\n    int x = 1;\n}\n"[..],
            // (2) A directive dragged onto the previous line stops being a
            // directive — the hazard Java does not have at all.
            b"class A {\n    int x = 1;\n#if FOO\n    int y = 2;\n#endif\n}\n",
        ] {
            let joined = emit::minimize_source(&joining, source).unwrap();
            assert!(
                verify::equivalent(&joining, source, &joined).is_err(),
                "the `false` branch should be refused for {source:?}"
            );
            // The shipped configuration keeps the line structure, so the same
            // source survives verification.
            let kept = emit::minimize_source(&csharp_config(), source).unwrap();
            verify::equivalent(&csharp_config(), source, &kept).unwrap();
        }
    }

    #[test]
    fn a_directive_argument_with_trailing_spaces_is_refused_not_rewritten() {
        // The single equivalence refusal the 880-file corpus produced under
        // the shipped configuration, pinned as behaviour. `#region helpers`
        // followed by spaces puts those spaces inside the `preproc_arg` leaf,
        // so collapsing the run moves the comparable artifact and the output
        // is discarded rather than written — safe, not silent. Without the
        // trailing spaces the same file is emitted and accepted.
        let config = csharp_config();
        let trailing = &b"class A {\n#region helpers   \n#endregion\n}\n"[..];
        let out = emit::minimize_source(&config, trailing).unwrap();
        assert!(verify::equivalent(&config, trailing, &out).is_err());
        let clean = &b"class A {\n#region helpers\n#endregion\n}\n"[..];
        let out = emit::minimize_source(&config, clean).unwrap();
        verify::equivalent(&config, clean, &out).unwrap();
    }

    #[test]
    fn a_slice_pattern_with_a_designation_is_refused_while_the_bare_form_parses() {
        let config = csharp_config();
        let bare = b"class A { int m(int[] xs) => xs switch { [var a, ..] => a, _ => 0 }; }\n";
        assert!(parse(&config, bare).is_ok());
        let designated =
            b"class A { int m(int[] xs) => xs switch { [var a, .. var r] => a, _ => 0 }; }\n";
        let err = parse(&config, designated).unwrap_err();
        assert!(
            err.to_string()
    ...
    fn preprocessor_directives_parse() {
        for source in [
            ...
            assert!(parse(&csharp_config(), source).is_ok(), "{source:?}");
        }
    }

    #[test]
    fn a_conditional_block_that_breaks_syntactic_structure_is_refused() {
        let config = csharp_config();
        for source in [
            &b"interface I<\n#if X\nout\n#endif\n T> {}\n"[..],
