...
use std::collections::BTreeMap;use std::fs;use std::io;use std::path::{Path,PathBuf};use std::process::{Command,Stdio};use std::sync::atomic::{AtomicU64,Ordering};use std::time::{SystemTime,UNIX_EPOCH};use tokenpress_core::{Error,Result};
...
const CSC_ARGS:[&str;6]=["/nologo","/noconfig","/nostdlib+","/t:library","/langversion:latest","/preferreduilang:en-US",];
/// The file names the fixtures are checked under; all four are private to a
/// `Scratch` directory.
const FORMATTED_NAME:&str="formatted.cs";const ORIGINAL_NAME:&str="original.cs";const SELF_TEST_NAME:&str="selftest.cs";const SELF_TEST_MINIMIZED_NAME:&str="selftest-minimized.cs";
...
#[derive(Clone,Debug,PartialEq,Eq)]struct Compiler{program:String,csc:PathBuf,}impl Compiler{
/// The command line, for quoting back in a diagnostic.
fn invocation(&self)->String{format!("{} {} {}",self.program,self.csc.display(),CSC_ARGS.join(" "))}}
/// What one checker run reported.
#[derive(Clone,Debug)]struct Report{
...
struct SystemTools;impl Tools for SystemTools{fn locate(&self,candidates:&[&str])->Option<Compiler>{candidates.iter().find_map(|name|{let listing=Command::new(name).arg(LIST_SDKS_ARG).stdin(Stdio::null()).stderr(Stdio::null()).output().ok()?;let text=String::from_utf8_lossy(&listing.stdout);let csc=sdk_compilers(&text).into_iter().rev().find(|p|p.is_file())?;Some(Compiler{program:(*name).to_string(),csc,})})}fn run(&self,compiler:&Compiler,dir:&Path,name:&str)->io::Result<Report>{let output=Command::new(&compiler.program).arg(&compiler.csc).args(CSC_ARGS).arg(name).current_dir(dir).stdin(Stdio::null()).output()?;let text=String::from_utf8_lossy(&output.stdout).trim().to_string();Ok(Report{codes:diagnostic_codes(&text),text,})}}
...
fn sdk_compilers(listing:&str)->Vec<PathBuf>{listing.lines().filter_map(|line|{let(version,rest)=line.trim().split_once(" [")?;let mut path=PathBuf::from(rest.strip_suffix(']')?);path.push(version);path.extend(CSC_RELATIVE);Some(path)}).collect()}
...
/// [`ERROR_MARKER`].
fn diagnostic_codes(output:&str)->Vec<String>{let mut codes:Vec<String> =output.lines().filter_map(|line|{let digits:String=line.split_once(ERROR_MARKER)?.1.chars().take_while(char::is_ascii_digit).collect();(!digits.is_empty()).then(| |format!("CS{digits}"))}).collect();codes.sort();codes}
/// The multiset difference between two sorted code lists: what the candidate
/// gained (`+`) and what it lost (`-`), for quoting back.
fn difference(before:&[String],after:&[String])->Vec<String>{let mut counts:BTreeMap<&str,i64> =BTreeMap::new();for code in after{*counts.entry(code.as_str()).or_default()+=1;}for code in before{*counts.entry(code.as_str()).or_default()-=1;}let mut gained:Vec<String> =Vec::new();let mut lost:Vec<String> =Vec::new();for(code,count)in counts{if count>0{gained.push(format!("+{count} {code}"));}if count<0{lost.push(format!("-{} {code}",-count));}}gained.extend(lost);gained}
...
pub fn check(original:&str,code:&str)->Result<()>{check_with(&SystemTools,original,code)}fn check_with(tools:&dyn Tools,original:&str,code:&str)->Result<()>{let compiler=tools.locate(&DOTNET_NAMES).ok_or_else(| |{Error::Verification("external verification needs the .NET SDK: no `dotnet` on PATH reporting an SDK \
             with a Roslyn `csc.dll` was found".to_string(),)})?;let scratch=Scratch::new()?;self_test(tools,&compiler,&scratch)?;scratch.write(ORIGINAL_NAME,original)?;let before=tools.run(&compiler,scratch.dir(),ORIGINAL_NAME)?;scratch.write(FORMATTED_NAME,code)?;let after=tools.run(&compiler,scratch.dir(),FORMATTED_NAME)?;if before.codes==after.codes{return Ok(());}Err(Error::Verification(format!("external check failed ({}): `csc` reports different diagnostics for the output than \
         for the input ({}), so formatting changed what the compiler sees: {}",compiler.invocation(),difference(&before.codes,&after.codes).join(", "),after.text)))}
/// Tests the gate before it is trusted; see the module docs on the self-test.
fn self_test(tools:&dyn Tools,compiler:&Compiler,scratch:&Scratch)->Result<()>{scratch.write(SELF_TEST_NAME,SELF_TEST_SOURCE)?;let before=tools.run(compiler,scratch.dir(),SELF_TEST_NAME)?;if!SELF_TEST_CODES.iter().all(|code|before.codes.iter().any(|seen|seen==code)){return Err(Error::Verification(format!("external verification cannot trust `{}`: its self-test file — valid C# in which \
             ...
             every other: {}",compiler.invocation(),SELF_TEST_CODES.join(" "),before.codes.join(" "),before.text)));}scratch.write(SELF_TEST_MINIMIZED_NAME,SELF_TEST_MINIMIZED)?;let after=tools.run(compiler,scratch.dir(),SELF_TEST_MINIMIZED_NAME)?;if before.codes!=after.codes{return Err(Error::Verification(format!("external verification cannot trust `{}`: its self-test pair — the same file before \
             and after this backend's own minimization — reported {} and {}, so the semantic \
             noise this gate relies on cancelling no longer cancels: {}",compiler.invocation(),before.codes.join(" "),after.codes.join(" "),after.text)));}Ok(())}
...
static SCRATCH_COUNTER:AtomicU64=AtomicU64::new(0);impl Scratch{fn new()->io::Result<Self>{let nanos=SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_nanos();let dir=std::env::temp_dir().join(format!("tokenpress-verify-csharp-{}-{nanos}-{}",std::process::id(),SCRATCH_COUNTER.fetch_add(1,Ordering::Relaxed)));fs::create_dir_all(&dir)?;Ok(Self{dir})}fn dir(&self)->&Path{&self.dir}fn write(&self,name:&str,contents:&str)->io::Result<()>{fs::write(self.dir.join(name),contents)}}impl Drop for Scratch{fn drop(&mut self){let _=fs::remove_dir_all(&self.dir);}}#[cfg(test)]mod tests{use super::*;use std::cell::RefCell;use std::collections::VecDeque;
/// One recorded [`Tools::run`] call, including what the file it was
/// pointed at contained at the time.
#[derive(Clone,Debug)]struct Call{dir:PathBuf,name:String,contents:String,}
...
struct FakeTools{available:&'static[&'static str],reports:RefCell<VecDeque<io::Result<Report>>>,calls:RefCell<Vec<Call>>,}impl FakeTools{fn new(available:&'static[&'static str],reports:Vec<io::Result<Report>>)->Self{Self{available,reports:RefCell::new(reports.into()),calls:RefCell::new(Vec::new()),}}fn calls(&self)->Vec<Call>{self.calls.borrow().clone()}}impl Tools for FakeTools{fn locate(&self,candidates:&[&str])->Option<Compiler>{candidates.iter().find(|name|self.available.contains(*name)).map(|name|Compiler{program:(*name).to_string(),csc:PathBuf::from("sdk").join("csc.dll"),})}fn run(&self,_compiler:&Compiler,dir:&Path,name:&str)->io::Result<Report>{self.calls.borrow_mut().push(Call{dir:dir.to_path_buf(),name:name.to_string(),contents:std::fs::read_to_string(dir.join(name)).expect("the checker's input must exist"),});self.reports.borrow_mut().pop_front().expect("more checker runs than the test scripted")}}
/// A scripted report, from the codes alone.
fn reports(codes:&[&str])->io::Result<Report>{Ok(Report{codes:codes.iter().map(|c|(*c).to_string()).collect(),text:codes.iter().map(|c|format!("a.cs(1,1): error {c}: something")).collect::<Vec<_>>().join("\n"),})}
/// What the self-test expects to see, as a scripted report.
fn self_test_passes()->io::Result<Report>{reports(&SELF_TEST_CODES)}
...
const BROKEN:&str="public class A\n{\nint F( { return 1; }\n}\n";#[test]fn no_dotnet_on_path_is_an_error_naming_the_tool(){let tools=FakeTools::new(&[],vec![]);let err=check_with(&tools,VALID,FORMATTED).unwrap_err();assert!(matches!(err,Error::Verification(_)),"{err}");let message=err.to_string();assert!(message.contains("dotnet"),"{message}");assert!(message.contains("PATH"),"{message}");assert!(tools.calls().is_empty());}#[test]fn csc_sees_the_self_test_pair_then_the_original_then_the_output(){let tools=FakeTools::new(&["dotnet"],vec![self_test_passes(),self_test_passes(),reports(&["CS0518"]),reports(&["CS0518"]),],);check_with(&tools,VALID,FORMATTED).unwrap();let calls=tools.calls();assert_eq!(calls.len(),4);for call in&calls{assert!(call.name.ends_with(".cs"),"{call:?}");assert!(call.dir.starts_with(std::env::temp_dir()),"{call:?}");assert!(!call.dir.exists(),"{call:?} outlived the check");assert_eq!(call.dir,calls[0].dir);}assert_eq!(calls[0].name,SELF_TEST_NAME);assert_eq!(calls[0].contents,SELF_TEST_SOURCE);assert_eq!(calls[1].name,SELF_TEST_MINIMIZED_NAME);assert_eq!(calls[1].contents,SELF_TEST_MINIMIZED);assert_eq!(calls[2].name,ORIGINAL_NAME);assert_eq!(calls[2].contents,VALID);assert_eq!(calls[3].name,FORMATTED_NAME);assert_eq!(calls[3].contents,FORMATTED);}#[test]fn a_self_test_whose_codes_were_not_extracted_stops_the_gate(){let tools=FakeTools::new(&["dotnet"],vec![reports(&[])]);let err=check_with(&tools,VALID,FORMATTED).unwrap_err();assert!(matches!(err,Error::Verification(_)),"{err}");let message=err.to_string();assert!(message.contains("self-test"),"{message}");assert!(message.contains("CS0246"),"{message}");assert!(message.contains("csc.dll"),"{message}");assert_eq!(tools.calls().len(),1);}#[test]fn a_self_test_whose_noise_stopped_cancelling_stops_the_gate(){let tools=FakeTools::new(&["dotnet"],vec![self_test_passes(),reports(&["CS0246","CS0518"])],);let err=check_with(&tools,VALID,FORMATTED).unwrap_err();assert!(matches!(err,Error::Verification(_)),"{err}");let message=err.to_string();assert!(message.contains("self-test"),"{message}");assert!(message.contains("cancel"),"{message}");assert_eq!(tools.calls().len(),2);}#[test]fn output_whose_diagnostics_differ_is_rejected(){let tools=FakeTools::new(&["dotnet"],vec![self_test_passes(),self_test_passes(),reports(&["CS0518"]),reports(&["CS1026"]),],);let err=check_with(&tools,VALID,BROKEN).unwrap_err();assert!(matches!(err,Error::Verification(_)),"{err}");let message=err.to_string();assert!(message.contains("external check failed"),"{message}");assert!(message.contains("+1 CS1026"),"{message}");assert!(message.contains("-1 CS0518"),"{message}");assert!(message.contains("error CS1026"),"{message}");}#[test]fn an_original_the_checker_already_rejects_is_not_blamed_on_the_formatter(){let tools=FakeTools::new(&["dotnet"],vec![self_test_passes(),self_test_passes(),reports(&["CS1021"]),reports(&["CS1021"]),],);check_with(&tools,"class Big\n{\n    long  x  =  99999999999999999999999;\n}\n","class Big\n{\nlong x = 99999999999999999999999;\n}\n",).unwrap();assert_eq!(tools.calls().len(),4);}#[test]fn a_checker_that_cannot_be_spawned_is_an_error(){let tools=FakeTools::new(&["dotnet"],vec![Err(io::Error::other("spawn failed"))]);let err=check_with(&tools,VALID,FORMATTED).unwrap_err();assert!(matches!(err,Error::Io(_)),"{err}");assert!(err.to_string().contains("spawn failed"),"{err}");}#[test]fn diagnostic_codes_are_a_multiset_with_positions_discarded(){let before="a.cs(1,7): error CS0246: nope\na.cs(9,3): error CS0518: nope\n";let after="a.cs(4,1): error CS0518: nope\na.cs(1,7): error CS0246: nope\n";assert_eq!(diagnostic_codes(before),["CS0246","CS0518"]);assert_eq!(diagnostic_codes(before),diagnostic_codes(after));let twice="a.cs(1,1): error CS0518: nope\na.cs(2,2): error CS0518: nope\n";assert_eq!(diagnostic_codes(twice),["CS0518","CS0518"]);}#[test]fn diagnostic_codes_read_errors_only_and_skip_what_they_cannot_parse(){let output="warning CS8021: No value for RuntimeMetadataVersion found.\n\
                      error CS0518: Predefined type 'System.Object' is not defined\n\
                      note: error CSnope\n\
                      \n";assert_eq!(diagnostic_codes(output),["CS0518"]);assert!(diagnostic_codes("").is_empty());}#[test]fn sdk_compilers_reads_the_list_sdks_listing(){let listing="8.0.129 [/usr/lib/dotnet/sdk]\n9.0.100 [/usr/lib/dotnet/sdk]\r\n";let found=sdk_compilers(listing);assert_eq!(found.len(),2);assert!(found[0].ends_with(PathBuf::from("8.0.129").join("Roslyn").join("bincore").join("csc.dll")));assert!(found[1].starts_with("/usr/lib/dotnet/sdk"));assert!(found[1].ends_with(PathBuf::from("9.0.100").join("Roslyn").join("bincore").join("csc.dll")));let windows=sdk_compilers("8.0.404 [C:\\Program Files\\dotnet\\sdk]\n");assert_eq!(windows.len(),1);assert!(windows[0].to_string_lossy().contains("C:\\Program Files\\dotnet\\sdk"),"{windows:?}");assert!(sdk_compilers("").is_empty());assert!(sdk_compilers("No SDKs were found.\n").is_empty());assert!(sdk_compilers("8.0.129 [/usr/lib/dotnet/sdk\n").is_empty());}#[test]fn difference_names_what_the_candidate_gained_and_lost(){let one=["CS0246".to_string()];let other=["CS1026".to_string()];assert_eq!(difference(&one,&other),["+1 CS1026","-1 CS0246"]);assert_eq!(difference(&[],&other),["+1 CS1026"]);assert_eq!(difference(&one,&[]),["-1 CS0246"]);let twice=["CS0518".to_string(),"CS0518".to_string()];let once=["CS0518".to_string()];assert_eq!(difference(&twice,&once),["-1 CS0518"]);assert!(difference(&twice,&twice).is_empty());}#[test]fn system_tools_locate_walks_the_candidates_in_order(){assert_eq!(SystemTools.locate(&["tokenpress-no-such-tool"]),None);let found=SystemTools.locate(&["tokenpress-no-such-tool","dotnet"]).expect("this machine must have a .NET SDK to run the suite");assert_eq!(found.program,"dotnet");assert!(found.csc.is_file(),"{found:?}");assert!(found.csc.ends_with("csc.dll"),"{found:?}");assert_eq!(SystemTools.locate(&DOTNET_NAMES),Some(found));}
/// The real compiler, located once per test that needs it.
fn system_compiler()->Compiler{SystemTools.locate(&DOTNET_NAMES).expect("this machine must have a .NET SDK to run the suite")}#[test]fn csc_writes_its_diagnostics_to_stdout(){let compiler=system_compiler();let scratch=Scratch::new().unwrap();scratch.write(SELF_TEST_NAME,SELF_TEST_SOURCE).unwrap();let output=Command::new(&compiler.program).arg(&compiler.csc).args(CSC_ARGS).arg(SELF_TEST_NAME).current_dir(scratch.dir()).stdin(Stdio::null()).output().unwrap();assert!(!output.status.success(),"{output:?}");assert_eq!(String::from_utf8_lossy(&output.stderr),"");let stdout=String::from_utf8_lossy(&output.stdout);assert!(stdout.contains("error CS0246"),"{stdout}");}#[test]fn the_self_test_fixture_provokes_the_codes_the_gate_looks_for(){let compiler=system_compiler();let scratch=Scratch::new().unwrap();scratch.write(SELF_TEST_NAME,SELF_TEST_SOURCE).unwrap();scratch.write(SELF_TEST_MINIMIZED_NAME,SELF_TEST_MINIMIZED).unwrap();let before=SystemTools.run(&compiler,scratch.dir(),SELF_TEST_NAME).unwrap();let after=SystemTools.run(&compiler,scratch.dir(),SELF_TEST_MINIMIZED_NAME).unwrap();let mut distinct=before.codes.clone();distinct.dedup();assert_eq!(distinct,SELF_TEST_CODES);assert_eq!(before.codes,after.codes);assert!(before.codes.len()>SELF_TEST_CODES.len());}#[test]fn the_self_test_pair_is_this_backends_own_output(){use std::path::Path;use tokenpress_core::{FormatOptions,Formatter};let produced=crate::CSharpFormatter::default().format(Path::new("Probe.cs"),SELF_TEST_SOURCE,&FormatOptions::default(),).unwrap();assert_eq!(produced.code,SELF_TEST_MINIMIZED);}#[test]fn system_tools_run_reports_the_diagnostic_multiset(){let compiler=system_compiler();let scratch=Scratch::new().unwrap();scratch.write("good.cs",VALID).unwrap();scratch.write("bad.cs",BROKEN).unwrap();let good=SystemTools.run(&compiler,scratch.dir(),"good.cs").unwrap();assert_eq!(good.codes,["CS0518","CS0518","CS0518"]);let bad=SystemTools.run(&compiler,scratch.dir(),"bad.cs").unwrap();assert_eq!(bad.codes,["CS1026"]);assert!(bad.text.starts_with("bad.cs("),"{}",bad.text);}#[test]fn langversion_latest_accepts_current_syntax(){let compiler=system_compiler();let scratch=Scratch::new().unwrap();scratch.write("modern.cs","class A\n{\n    int[] xs = [1, 2];\n}\n").unwrap();let report=SystemTools.run(&compiler,scratch.dir(),"modern.cs").unwrap();assert!(CSC_ARGS.contains(&"/langversion:latest"));for code in&report.codes{assert_eq!(code,"CS0518","{}",report.text);}}#[test]fn csc_accepts_output_it_still_reads_the_same_way(){check_with(&SystemTools,VALID,FORMATTED).unwrap();}#[test]fn csc_accepts_output_whose_references_cannot_be_resolved(){check_with(&SystemTools,SELF_TEST_SOURCE,SELF_TEST_MINIMIZED).unwrap();}#[test]fn csc_rejects_output_with_an_introduced_syntax_error(){let err=check_with(&SystemTools,VALID,BROKEN).unwrap_err();let message=err.to_string();assert!(message.contains("external check failed"),"{message}");assert!(message.contains("csc.dll"),"{message}");assert!(message.contains("+1 CS1026"),"{message}");assert!(message.contains(") expected"),"{message}");}#[test]fn csc_rejects_a_top_level_statement_after_a_type_declaration(){let err=check_with(&SystemTools,"class A { }\n","class A { }\nint x = 1;\n").unwrap_err();let message=err.to_string();assert!(message.contains("+1 CS8803"),"{message}");assert!(message.contains("Top-level statements must precede"),"{message}");}#[test]fn input_csc_already_rejects_does_not_fail_the_run(){check_with(&SystemTools,"class Big\n{\n    long  x  =  99999999999999999999999;\n}\n","class Big\n{\nlong x = 99999999999999999999999;\n}\n",).unwrap();}#[test]fn check_runs_the_real_csc(){check(VALID,FORMATTED).unwrap();let err=check(VALID,BROKEN).unwrap_err();assert!(err.to_string().contains("external check failed"),"{err}");}#[test]fn scratch_directories_are_unique_and_removed_on_drop(){let first=Scratch::new().unwrap();let second=Scratch::new().unwrap();assert_ne!(first.dir,second.dir);first.write("A.cs",VALID).unwrap();assert_eq!(std::fs::read_to_string(first.dir().join("A.cs")).unwrap(),VALID);let dir=first.dir.clone();drop(first);assert!(!dir.exists());}}