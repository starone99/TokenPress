...
use crate::comparable;use crate::parser;use tokenpress_core::{Error,Result};
/// Weakest level: the output must parse at all.
pub fn reparse(output:&[u8])->Result<()>{parser::parse(output).map_err(reparse_failure)?;Ok(())}
...
pub fn equivalent(original:&[u8],output:&[u8])->Result<()>{let expected=comparable::comparable(original)?;let actual=comparable::comparable(output).map_err(reparse_failure)?;if expected!=actual{return Err(Error::Verification("output AST differs from input".to_string(),));}Ok(())}
/// Restates a parse failure of the *output* as the verifier's refusal.
fn reparse_failure(error:Error)->Error{Error::Verification(format!("output failed to re-parse: {error}"))}#[cfg(test)]mod tests{use super::*;use crate::emit;#[test]fn reparse_accepts_valid_and_rejects_invalid_output(){assert!(reparse(b"def f(a, b)\n  a + b\nend\n").is_ok());let err=reparse(b"def ; end").unwrap_err();assert!(matches!(err,Error::Verification(_)),"{err}");assert!(err.to_string().starts_with("verification failed: output failed to re-parse: "),"{err}");}#[test]fn reparse_does_not_gate_on_warnings(){assert!(reparse(b"a = 0\nif a = 1\nend\n").is_ok());}#[test]fn reparse_accepts_non_utf8_output(){assert!(reparse(b"# encoding: binary\nx = \"\xff\xfe\"\n").is_ok());}#[test]fn a_minimized_hazard_source_is_equivalent_to_its_input(){let source=b"class A\n  def b(c)\n    s = \"a#{ c } b\"\n    xs = %w[a   b]\n    xs.each do |x|\n      p x   # note\n    end\n    s\n  end\nend\n";let output=emit::minimize_source(source).unwrap();assert!(output.len()<source.len());assert!(equivalent(source,&output).is_ok());}#[test]fn a_stripped_source_is_equivalent_to_its_input(){let source=b"# frozen_string_literal: true\n# leading note\nx = 1  # trailing note\ny = 2\n";let output=emit::strip_comments_source(source).unwrap();assert!(equivalent(source,&output).is_ok());}#[test]fn semantic_changes_are_rejected(){for(name,original,output)in[("integer value",&b"x = 1\n"[..],&b"x = 2\n"[..]),("unary argument vs binary operator",b"a - b\n",b"a -b\n"),("quote style",b"x = \"a\"\n",b"x = 'a'\n"),]{let err=equivalent(original,output).unwrap_err();assert!(matches!(err,Error::Verification(_)),"{name}: {err}");assert_eq!(err.to_string(),"verification failed: output AST differs from input","{name}");}}#[test]fn unparsable_output_is_rejected_by_the_equivalence_check(){let err=equivalent(b"x = 1\n",b"1 +").unwrap_err();assert!(matches!(err,Error::Verification(_)),"{err}");assert!(err.to_string().starts_with("verification failed: output failed to re-parse: "),"{err}");}#[test]fn an_unparsable_input_is_a_parse_error_not_a_refusal(){let err=equivalent(b"def ; end",b"x = 1\n").unwrap_err();assert!(matches!(err,Error::Parse(_)),"{err}");}#[test]fn non_utf8_output_is_verified_on_its_bytes(){let source=b"# encoding: binary\nx  =  \"\xff\xfe\"\n";let output=emit::minimize_source(source).unwrap();assert!(equivalent(source,&output).is_ok());assert!(equivalent(source,b"# encoding: binary\nx = \"\xfe\xff\"\n").is_err());}#[test]fn a_squiggly_heredoc_reindent_is_a_known_over_refusal(){let err=equivalent(b"x = <<~A\n    hi\n  A\n",b"x = <<~A\n  hi\nA\n").unwrap_err();assert_eq!(err.to_string(),"verification failed: output AST differs from input");}#[test]fn a_multiline_index_call_is_a_known_over_refusal(){assert!(equivalent(b"a[1,\n2]\n",b"a[1, 2]\n").is_err());}#[test]fn deleting_a_buried_magic_looking_comment_is_a_known_over_refusal(){let source=b"x = 1\n# encoding: binary\ny = 2\n";let output=emit::strip_comments_source(source).unwrap();assert!(!output.windows(8).any(|w|w==b"encoding"));let err=equivalent(source,&output).unwrap_err();assert_eq!(err.to_string(),"verification failed: output AST differs from input");}}