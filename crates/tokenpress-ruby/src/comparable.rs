...
use std::mem::MaybeUninit;use ruby_prism_sys::{pm_node_destroy,pm_node_t,pm_parse,pm_parser_free,pm_parser_init,pm_parser_t,};use tokenpress_core::Result;
...
const SEMANTIC_MAGIC_COMMENT_KEYS:[&str;4]=["coding","encoding","frozen_string_literal","shareable_constant_value",];
...
#[repr(C)]struct Buffer{length:usize,capacity:usize,value:*mut std::os::raw::c_char,}unsafe extern "C"{fn pm_buffer_init(buffer:*mut Buffer)->bool;fn pm_buffer_free(buffer:*mut Buffer);fn pm_prettyprint(output_buffer:*mut Buffer,parser:*const pm_parser_t,node:*const pm_node_t,);}
...
pub fn comparable(source:&[u8])->Result<String>{let parsed=crate::parser::parse(source)?;let mut artifact=String::new();for magic in parsed.magic_comments(){let mut key=String::new();push_escaped(&mut key,magic.key());key.make_ascii_lowercase();if SEMANTIC_MAGIC_COMMENT_KEYS.contains(&key.as_str()){artifact.push_str("magic ");artifact.push_str(&key);artifact.push('=');push_escaped(&mut artifact,magic.value());artifact.push('\n');}}artifact.push_str("---\n");for line in prettyprint(source).split_inclusive(|byte|*byte==b'\n'){push_escaped(&mut artifact,&strip_location(line));}Ok(artifact)}
/// Convenience wrapper: `true` when `a` and `b` have the same artifact.
///
/// Propagates the parse error if *either* side fails to parse.
pub fn equivalent(a:&[u8],b:&[u8])->Result<bool>{Ok(comparable(a)?==comparable(b)?)}
...
/// never asked for.
fn prettyprint(source:&[u8])->Vec<u8>{let uninit=Box::into_raw(Box::new(MaybeUninit::<pm_parser_t>::uninit()));unsafe{pm_parser_init((*uninit).as_mut_ptr(),source.as_ptr(),source.len(),std::ptr::null(),);let parser:*mut pm_parser_t=(*uninit).assume_init_mut();let node=pm_parse(parser);let mut buffer=MaybeUninit::<Buffer>::uninit();assert!(pm_buffer_init(buffer.as_mut_ptr()),"prism buffer allocation");let mut buffer=buffer.assume_init();pm_prettyprint(&mut buffer,parser,node);let rendered=std::slice::from_raw_parts(buffer.value.cast::<u8>(),buffer.length).to_vec();pm_buffer_free(&mut buffer);pm_node_destroy(parser,node);pm_parser_free(parser);drop(Box::from_raw(uninit));rendered}}
...
fn strip_location(line:&[u8])->Vec<u8>{if let Some(separator)=line.windows(2).position(|pair|pair==b": "){let value=separator+2;if let Some(length)=coordinate_tuple_len(&line[value..]){return[&line[..value],&line[value+length..]].concat();}}line.to_vec()}
...
fn coordinate_tuple_len(value:&[u8])->Option<usize>{const SHAPE:&[u8]=b"(,)-(,)";let mut index=0;for expected in SHAPE{if value.get(index)!=Some(expected){return None;}index+=1;if!matches!(*expected,b')'|b'-'){let digits=value[index..].iter().take_while(|byte|byte.is_ascii_digit()).count();if digits==0{return None;}index+=digits;}}Some(index)}
/// Appends `bytes` to `out`, escaping everything outside ASCII as `\xNN`.
fn push_escaped(out:&mut String,bytes:&[u8]){for byte in bytes{if byte.is_ascii(){out.push(char::from(*byte));}else{out.push_str(&format!("\\x{byte:02X}"));}}}#[cfg(test)]mod tests{use super::*;use tokenpress_core::Error;
/// Pairs that differ only in formatting: the artifact must be identical.
const EQUIVALENT:&[(&str,&str,&str)]=&[("whitespace only","x  =   1\n","x = 1\n"),("newline vs semicolon","a = 1\nb = 2\n","a = 1; b = 2\n"),("indentation removed","def f\n    1\nend\n","def f\n1\nend\n",),("blank line runs","a = 1\n\n\n\nb = 2\n","a = 1\nb = 2\n"),("trailing whitespace","x = 1   \n","x = 1\n"),("final newline","x = 1","x = 1\n"),("joined argument list","foo(1,\n  2)\n","foo(1, 2)\n"),("joined parameter list","def f(a,\nb)\nend\n","def f(a, b)\nend\n",),("hash label spacing","f(a: 1)\n","f(a:1)\n"),("word list spacing","%w[a   b]\n","%w[a b]\n"),("symbol list spacing","%i[a   b]\n","%i[a b]\n"),("operator spacing","1 + 2\n","1+2\n"),("comment added","x = 1\n","# note\nx = 1\n"),("comment inside a body","def f\n  # hi\n  1\nend\n","def f\n  1\nend\n",),("embdoc removed","=begin\nhi\n=end\nx = 1\n","x = 1\n"),("brace block reflowed","xs.each { |x| p x }\n","xs.each { |x|\n  p x\n}\n",),("do block reflowed","xs.each do |x|\n  p x\nend\n","xs.each do |x| p x end\n",),("class body reflowed","class A\n  def b; 1; end\nend\n","class A\ndef b\n1\nend\nend\n",),("module body reflowed","module M\n  X = 1\nend\n","module M\nX = 1\nend\n",),("rescue clause reflowed","begin\n  a\nrescue => e\n  b\nend\n","begin\na\nrescue => e\nb\nend\n",),];
/// Pairs that must never be accepted as equivalent.
const DIFFERENT:&[(&str,&str,&str)]=&[("integer value","x = 1\n","x = 2\n"),("integer base flag","x = 0x10\n","x = 16\n"),("binary integer base flag","x = 0b10\n","x = 2\n"),("integer vs float","x = 1\n","x = 1.0\n"),("different operator","1 + 2\n","1 - 2\n"),("unary argument vs binary operator","a - b\n","a -b\n"),("parenthesized argument","foo(1..2)\n","foo (1..2)\n"),("string vs symbol","x = \"a\"\n","x = :a\n"),("string contents","x = \"a\"\n","x = 'b'\n"),("quote style","x = \"a\"\n","x = 'a'\n"),("interpolated string contents","x = \"a#{1}b\"\n","x = \"a#{1}c\"\n",),("regexp contents","x = /a/\n","x = /b/\n"),("word list vs symbol list","%w[a b]\n","%i[a b]\n"),("method rename","def a; end\n","def b; end\n"),("added statement","a = 1\n","a = 1\nb = 2\n"),("removed statement","a = 1\nb = 2\n","a = 1\n"),("heredoc contents","x = <<A\nhi\nA\n","x = <<A\nho\nA\n"),("safe navigation flag","a&.b\n","a.b\n"),("range exclusivity","1..2\n","1...2\n"),("then keyword","if a\n  b\nend\n","if a then b end\n"),("endless method definition","def a = 1\n","def a; 1; end\n",),("frozen string literal with strings","# frozen_string_literal: true\nx = \"a\"\n","x = \"a\"\n",),("frozen string literal without strings","# frozen_string_literal: true\nx = 1\n","x = 1\n",),("frozen string literal true vs false","# frozen_string_literal: true\nx = 1\n","# frozen_string_literal: false\nx = 1\n",),("encoding magic comment","# encoding: binary\nx = 1\n","x = 1\n",),("shareable constant value magic comment","# shareable_constant_value: literal\nX = [1]\n","X = [1]\n",),("string that looks like a location","x = \"(1,2)-(3,4)\"\n","x = \"(9,9)-(9,9)\"\n",),];#[test]fn formatting_only_changes_compare_equal(){for(name,left,right)in EQUIVALENT{let left=comparable(left.as_bytes()).unwrap();let right=comparable(right.as_bytes()).unwrap();assert_eq!(left,right,"{name}: expected equal");}}#[test]fn semantic_changes_compare_different(){for(name,left,right)in DIFFERENT{let left=comparable(left.as_bytes()).unwrap();let right=comparable(right.as_bytes()).unwrap();assert_ne!(left,right,"{name}: expected different");}}#[test]fn a_squiggly_heredoc_reindent_is_an_over_refusal(){assert!(!equivalent(b"x = <<~A\n    hi\n  A\n",b"x = <<~A\n  hi\nA\n").unwrap());}#[test]fn a_multiline_index_call_is_an_over_refusal(){assert!(!equivalent(b"a[1,\n2]\n",b"a[1, 2]\n").unwrap());}#[test]fn coordinate_tuples_are_stripped(){let artifact=comparable(b"x = 1\n").unwrap();assert!(artifact.contains("@ ProgramNode (location: )"),"{artifact}");assert!(!artifact.contains("(1,0)"),"{artifact}");assert!(artifact.contains("+-- operator_loc:  = \"=\""),"{artifact}");}#[test]fn the_magic_comment_prelude_lists_only_semantic_keys(){let artifact=comparable(b"# frozen_string_literal: true\n# note: hi\nx = 1\n").unwrap();let(prelude,_)=artifact.split_once("\n---\n").unwrap();assert_eq!(prelude,"magic frozen_string_literal=true");}#[test]fn the_magic_comment_prelude_is_empty_when_there_are_none(){let artifact=comparable(b"# note: hi\nx = 1\n").unwrap();assert!(artifact.starts_with("---\n"),"{artifact}");}#[test]fn magic_comment_keys_are_matched_case_insensitively(){let artifact=comparable(b"# Frozen_String_Literal: true\nx = 1\n").unwrap();assert!(artifact.starts_with("magic frozen_string_literal=true\n"),"{artifact}");}#[test]fn non_ascii_bytes_are_escaped(){let artifact=comparable("def ほげ; end\n".as_bytes()).unwrap();assert!(artifact.is_ascii(),"{artifact}");assert!(artifact.contains("+-- name: :\\xE3\\x81\\xBB\\xE3\\x81\\x92"),"{artifact}");}#[test]fn non_utf8_sources_are_accepted_and_distinguished(){let left=b"# encoding: binary\nx = \"\xff\"\n".to_vec();let right=b"# encoding: binary\nx = \"\xfe\"\n".to_vec();assert!(comparable(&left).unwrap().is_ascii());assert!(!equivalent(&left,&right).unwrap());}#[test]fn a_parse_error_is_reported_not_rendered(){let err=comparable(b"def ; end").unwrap_err();assert!(matches!(err,Error::Parse(_)),"{err}");}#[test]fn equivalent_propagates_a_parse_error_from_either_side(){assert!(matches!(equivalent(b"def ; end",b"x = 1\n").unwrap_err(),Error::Parse(_)));assert!(matches!(equivalent(b"x = 1\n",b"1 +").unwrap_err(),Error::Parse(_)));}#[test]fn an_empty_source_has_an_artifact(){assert!(equivalent(b"",b"\n\n").unwrap());assert!(comparable(b"").unwrap().contains("@ ProgramNode"));}#[test]fn the_artifact_is_deterministic(){let source=b"class A\n  def b(c) = c + 1\nend\n";assert_eq!(comparable(source).unwrap(),comparable(source).unwrap());}}