...
impl RubyFormatter {
    pub fn new(options: RubyOptions) -> Self {
        Self { options }
    }
}
...
impl Formatter for RubyFormatter {
    ...
    fn format(&self, _path: &Path, source: &str, options: &FormatOptions) -> Result<FormatResult> {
        // The parse borrows `bytes`, so emit and verify have to run here with
        // it; see the borrow rule in [`parser`].
        let bytes = source.as_bytes();
        let parsed = parser::parse(bytes)?;
        let emitted = if self.options.strip_comments {
            let plan = emit::strip_comments_plan(&parsed);
            emit::strip_comments(bytes, &plan, emit::minimize)
        } else {
            let spans = emit::protected_spans(&parsed);
            emit::rewrite(bytes, &spans, emit::minimize)
        };
        // The emitter only ever copies input bytes and drops whole spans or
        // runs of ASCII whitespace, so valid UTF-8 in yields valid UTF-8 out
        // and this conversion is lossless. Spelling it `_lossy` keeps that
        // invariant from becoming an error branch no input can reach — and
        // costs nothing even if it were ever wrong, because verification runs
        // on the converted bytes below: a substitution would move the AST and
...
mod tests {
    ...
    fn fmt(source: &str) -> String {
        fmt_with(source, RubyOptions::default())
    }

    fn fmt_with(source: &str, options: RubyOptions) -> String {
        RubyFormatter::new(options)
            .format(Path::new("a.rb"), source, &FormatOptions::default())
            .unwrap()
            .code
    }
    ...
    #[test]
    fn language_is_ruby() {
        assert_eq!(RubyFormatter::default().language(), "ruby");
    }

    #[test]
    fn supports_the_ruby_paths() {
        // The decision itself lives in `paths`, which owns its own table of
        // cases; this only pins that the formatter delegates to it — including
        // the extensionless build files no other backend has to think about.
        let f = RubyFormatter::default();
        for name in [
            "a.rb",
            "tasks.rake",
            "tokenpress.gemspec",
        ...
        for name in ["a.py", "a.rs", "a.txt", "gemfile", "Gemfile.lock", "rb"] {
            assert!(!f.supports(Path::new(name)), "{name} should be rejected");
        }
    }

    #[test]
    fn rb01_minimizes_whitespace() {
    ...
    fn the_path_is_not_read() {
        // prism has no dialect, filepath or version selector, so a supported
        // path is the caller's filter and nothing more: `format` handed an
        // unsupported name still formats the source it was given as Ruby.
        let r = RubyFormatter::default()
            .format(
                Path::new("notes.txt"),
                "x  =  1\n",
    ...
    #[test]
    fn rbo1_defaults_to_keeping_comments() {
        assert!(!RubyOptions::default().strip_comments);
    }

    #[test]
    fn rbo1_keeps_every_comment_by_default() {
        // Leading, trailing and embdoc alike — the whole difference from the
        // JavaScript backend, so all three shapes are pinned in one case.
        let source = "=begin\nblock\n=end\n# leading\nx  =  1  # trailing\n";
        assert_eq!(
            fmt(source),
            "=begin\nblock\n=end\n# leading\nx = 1 # trailing\n"
    ...
    #[test]
    fn rbo1_strips_comments_on_request() {
        // Everything here sits after the first code token, so nothing is in
        // the magic-comment window and all three shapes go — a comment line
        // takes its whole line with it, a trailing one leaves its code alone.
    ...
    #[test]
    fn rbo1_keeps_the_magic_comment_window_when_stripping() {
    ...
    fn parse_errors_are_reported() {
        ...
        assert!(err.to_string().starts_with("parse error: byte "), "{err}");
    }

    #[test]
    fn a_multiline_index_call_is_a_known_over_refusal() {
        // The one over-refusal the whitespace policy triggers: `message_loc`
        // for `[]` spans the whole bracket pair, so removing the indentation
        // inside it moves a source slice `verify` compares. The file is left
        // alone — a saving lost, never a corrupted write.
        let err = RubyFormatter::default()
            .format(Path::new("a.rb"), "a[1,\n  2]\n", &FormatOptions::default())
            .unwrap_err();
        assert_eq!(
