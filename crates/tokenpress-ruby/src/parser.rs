...
pub use ruby_prism::{Comment, CommentType, Location, Node, ParseResult, Visit};
...
pub fn parse(source: &[u8]) -> Result<ParseResult<'_>> {
    let parsed = ruby_prism::parse(source);
    let details = parsed
        .errors()
        .map(|e| format!("byte {}: {}", e.location().start_offset(), e.message()))
        .collect::<Vec<_>>();
    if !details.is_empty() {
        return Err(Error::Parse(details.join("; ")));
    }
    Ok(parsed)
}
...
mod tests {
    ...
    #[test]
    fn parses_valid_ruby() {
        let parsed = parse(b"def f(a, b)\n  a + b\nend\n").unwrap();
        assert_eq!(parsed.errors().count(), 0);
        assert!(matches!(parsed.node(), Node::ProgramNode { .. }));
    }

    #[test]
    fn warnings_do_not_reject() {
        // Valid Ruby that prism warns about: an assignment used as a
        // condition. Gating on warnings would refuse a working file.
        let parsed = parse(b"a = 0\nif a = 1\nend\n").unwrap();
        assert!(parsed.warnings().count() > 0);
    }

    #[test]
    fn an_unused_variable_also_only_warns() {
    ...
    fn rejects_a_syntax_error() {
        let err = parse(b"def ; end").unwrap_err();
        assert!(matches!(err, Error::Parse(_)), "{err}");
        let message = err.to_string();
        assert!(message.starts_with("parse error: byte "), "{message}");
        // The prism detail has to survive into the message, not just the
        // offset prefix.
        assert!(message.len() > "parse error: byte 0: ".len(), "{message}");
    }
    ...
    #[test]
    fn accepts_non_utf8_sources() {
        // prism parses bytes, so a source that is not valid UTF-8 is still a
        // valid parse input as long as Ruby itself considers it valid: the
        // magic comment picks the encoding the literal is read in. Refusing
        // non-UTF-8 input is the Formatter layer's job (its contract is
        // `&str`), not this one's.
        for source in [
            b"# encoding: binary\nx = \"\xff\xfe\"\n".to_vec(),
            b"# encoding: shift_jis\nx = \"\x82\xa0\"\n".to_vec(),
            // Non-UTF-8 bytes inside a comment need no magic comment at all.
            b"# \xe9\nx = 1\n".to_vec(),
        ] {
    ...
    #[test]
    fn a_stray_non_utf8_byte_without_a_magic_comment_is_a_parse_error() {
        // The flip side of the above, pinned so the distinction is not read
        // as "any bytes go": the default encoding is UTF-8, and `0xFF` is not
        // valid UTF-8, so this is a genuine Ruby syntax error rather than
        // something this module chooses to refuse.
        let err = parse(b"x = \"\xff\xfe\"\n").unwrap_err();
        assert!(matches!(err, Error::Parse(_)), "{err}");
    }

    #[test]
    fn the_result_borrows_the_source() {
        // Pins the borrow rule: locations index back into the caller's bytes.
        let source = b"x = 1\n";
        let parsed = parse(source).unwrap();
