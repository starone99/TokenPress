...
const FORMATTED_NAME: &str = "formatted.rb";
const ORIGINAL_NAME: &str = "original.rb";
...
impl Tools for SystemTools {
    ...
    fn run(&self, program: &str, args: &[&str], file: &Path) -> io::Result<Outcome> {
        ...
        // A file that did not compile has its diagnostics on stderr; the
        // `Syntax OK` line stdout carries belongs to the branch above.
        Ok(Outcome::Fail(
...
fn check_with(tools: &dyn Tools, original: &str, code: &str) -> Result<()> {
    let program = tools.locate(&RUBY_NAMES).ok_or_else(|| {
        Error::Verification(
            "external verification needs `ruby` on PATH: it was not found".to_string(),
        )
    })?;
    let scratch = Scratch::new()?;

    let before = scratch.write(ORIGINAL_NAME, original)?;
    if let Outcome::Fail(_) = tools.run(&program, &RUBY_ARGS, &before)? {
    ...
    match tools.run(&program, &RUBY_ARGS, &after)? {
        Outcome::Pass => Ok(()),
        Outcome::Fail(message) => Err(Error::Verification(format!(
            "external check failed ({program} -c): {message}"
        ))),
    }
}
...
mod tests {
    ...
    #[test]
    fn no_ruby_on_path_is_an_error_naming_the_tool() {
        let tools = FakeTools::new(&[], vec![]);
        let err = check_with(&tools, "x  =  1\n", "x = 1\n").unwrap_err();
    ...
    #[test]
    fn ruby_sees_the_original_first_and_then_the_output() {
        let tools = FakeTools::new(&["ruby"], vec![pass(), pass()]);
        check_with(&tools, "x  =  1\n", "x = 1\n").unwrap();
        let calls = tools.calls();
        assert_eq!(calls.len(), 2);
        for call in &calls {
            assert_eq!(call.program, "ruby");
            assert_eq!(call.args, ["-c"]);
            // A `.rb` file in a private directory, never the user's own path.
            assert_eq!(call.file.extension().unwrap(), "rb");
        ...
        assert_eq!(calls[0].contents, "x  =  1\n");
    ...
    fn output_that_fails_the_external_check_is_rejected() {
        let tools = FakeTools::new(&["ruby"], vec![pass(), fail("a.rb:1: syntax error")]);
        let err = check_with(&tools, "x  =  1\n", "x =\n").unwrap_err();
        assert!(matches!(err, Error::Verification(_)), "{err}");
        let message = err.to_string();
        assert!(message.contains("ruby -c"), "{message}");
        assert!(message.contains("a.rb:1: syntax error"), "{message}");
    }
    ...
    fn an_original_that_already_fails_is_not_blamed_on_the_formatter() {
        ...
        let tools = FakeTools::new(&["ruby"], vec![fail("a.rb:1: empty range in char class")]);
        check_with(&tools, "x  =  /[z-a]/\n", "x = /[z-a]/\n").unwrap();
    ...
    fn a_checker_that_cannot_be_spawned_is_an_error() {
        let tools = FakeTools::new(&["ruby"], vec![Err(io::Error::other("spawn failed"))]);
        let err = check_with(&tools, "x  =  1\n", "x = 1\n").unwrap_err();
    ...
    fn system_tools_locate_walks_the_candidates_in_order() {
        ...
        assert_eq!(
            SystemTools.locate(&["tokenpress-no-such-tool", "ruby"]),
            Some("ruby".to_string())
        );
        ...
        assert_eq!(SystemTools.locate(&RUBY_NAMES), Some("ruby".to_string()));
    }

    #[test]
    fn system_tools_run_reports_rubys_verdict() {
        let scratch = Scratch::new().unwrap();
        let good = scratch.write("good.rb", "x = 1\n").unwrap();
        let bad = scratch.write("bad.rb", "x =\n").unwrap();
        assert_eq!(
            verdict(SystemTools.run("ruby", &RUBY_ARGS, &good).unwrap()),
            ""
        );
        let message = verdict(SystemTools.run("ruby", &RUBY_ARGS, &bad).unwrap());
        assert!(message.contains("syntax error"), "{message}");
    }

    #[test]
    fn warnings_on_stderr_do_not_fail_the_check() {
        // `ruby -c` warns about a duplicated hash key and still exits 0. The
        // gate is the exit status alone, so a file that only warns passes.
        let scratch = Scratch::new().unwrap();
        let warns = scratch.write("warns.rb", "h = {a: 1, a: 2}\n").unwrap();
        assert_eq!(
            verdict(SystemTools.run("ruby", &RUBY_ARGS, &warns).unwrap()),
            ""
        );
        // ... and the same file goes through the orchestration untouched.
        check_with(&SystemTools, "h  =  {a: 1, a: 2}\n", "h = {a: 1, a: 2}\n").unwrap();
    }

    #[test]
    fn ruby_accepts_output_it_can_still_parse() {
        check_with(
            &SystemTools,
            "def add(a, b)\n    a + b\nend\n",
            "def add(a, b)\na + b\nend\n",
    ...
    #[test]
    fn ruby_rejects_output_it_cannot_parse() {
        let err = check_with(&SystemTools, "x  =  1\n", "x =\n").unwrap_err();
        let message = err.to_string();
        assert!(
            message.contains("external check failed (ruby -c)"),
            "{message}"
        );
        assert!(message.contains("syntax error"), "{message}");
    }
    ...
    #[test]
    fn input_ruby_already_rejects_does_not_fail_the_run() {
        // `/[z-a]/`: a well-formed regexp *literal*, which prism parses and
        // MRI refuses to compile ("empty range in char class"). The formatted
        // output would be refused for exactly the same reason, so the policy
        // is what keeps this from being reported as a TokenPress failure.
        check_with(&SystemTools, "x  =  /[z-a]/\n", "x = /[z-a]/\n").unwrap();
    }

    #[test]
    fn check_runs_the_real_interpreter() {
        check("x  =  1\n", "x = 1\n").unwrap();
        let err = check("x  =  1\n", "x =\n").unwrap_err();
