...
const FORMATTED_NAME:&str="formatted.go";const ORIGINAL_NAME:&str="original.go";
...
struct SystemTools;impl Tools for SystemTools{fn locate(&self,candidates:&[&str])->Option<String>{candidates.iter().find(|name|{Command::new(name).args(GOFMT_PROBE_ARGS).stdin(Stdio::null()).stdout(Stdio::null()).stderr(Stdio::null()).status().is_ok()}).map(|name|(*name).to_string())}fn run(&self,program:&str,args:&[&str],file:&Path)->io::Result<Outcome>{let output=Command::new(program).args(args).arg(file).stdin(Stdio::null()).stdout(Stdio::null()).output()?;if output.status.success(){return Ok(Outcome::Pass);}Ok(Outcome::Fail(String::from_utf8_lossy(&output.stderr).trim().to_string(),))}}
...
pub fn check(original:&str,code:&str)->Result<()>{check_with(&SystemTools,original,code)}fn check_with(tools:&dyn Tools,original:&str,code:&str)->Result<()>{let program=tools.locate(&GOFMT_NAMES).ok_or_else(| |{Error::Verification("external verification needs `gofmt` on PATH: it was not found".to_string(),)})?;let scratch=Scratch::new()?;let before=scratch.write(ORIGINAL_NAME,original)?;if let Outcome::Fail(_)=tools.run(&program,&GOFMT_ARGS,&before)?{return Ok(());}let after=scratch.write(FORMATTED_NAME,code)?;match tools.run(&program,&GOFMT_ARGS,&after)?{Outcome::Pass=>Ok(()),Outcome::Fail(message)=>Err(Error::Verification(format!("external check failed ({program} -e): {message}"))),}}
...
fn verdict(outcome:Outcome)->String{match outcome{Outcome::Pass=>String::new(),Outcome::Fail(message)=>message,}}const VALID:&str="package main\n\nfunc main() {}\n";const FORMATTED:&str="package main\nfunc main() {}\n";#[test]fn no_gofmt_on_path_is_an_error_naming_the_tool(){let tools=FakeTools::new(&[],vec![]);let err=check_with(&tools,VALID,FORMATTED).unwrap_err();assert!(matches!(err,Error::Verification(_)),"{err}");let message=err.to_string();assert!(message.contains("gofmt"),"{message}");assert!(message.contains("PATH"),"{message}");assert!(tools.calls().is_empty());}#[test]fn gofmt_sees_the_original_first_and_then_the_output(){let tools=FakeTools::new(&["gofmt"],vec![pass(),pass()]);check_with(&tools,VALID,FORMATTED).unwrap();let calls=tools.calls();assert_eq!(calls.len(),2);for call in&calls{assert_eq!(call.program,"gofmt");assert_eq!(call.args,["-e"]);assert_eq!(call.file.extension().unwrap(),"go");assert!(call.file.starts_with(std::env::temp_dir()),"{call:?}");assert!(!call.file.exists(),"{call:?} outlived the check");}assert_eq!(calls[0].contents,VALID);assert_eq!(calls[1].contents,FORMATTED);assert_eq!(calls[0].file.parent(),calls[1].file.parent());assert_ne!(calls[0].file,calls[1].file);}#[test]fn output_that_fails_the_external_check_is_rejected(){let tools=FakeTools::new(&["gofmt"],vec![pass(),fail("a.go:3:15: expected ';', found 'EOF'")],);let err=check_with(&tools,VALID,"package main\nfunc main() {\n").unwrap_err();assert!(matches!(err,Error::Verification(_)),"{err}");let message=err.to_string();assert!(message.contains("gofmt -e"),"{message}");assert!(message.contains("expected ';', found 'EOF'"),"{message}");}#[test]fn an_original_that_already_fails_is_not_blamed_on_the_formatter(){let tools=FakeTools::new(&["gofmt"],vec![fail("a.go:1:1: expected 'package'")]);check_with(&tools,"\n\n// note\n","// note\n").unwrap();assert_eq!(tools.calls().len(),1);}#[test]fn a_checker_that_cannot_be_spawned_is_an_error(){let tools=FakeTools::new(&["gofmt"],vec![Err(io::Error::other("spawn failed"))]);let err=check_with(&tools,VALID,FORMATTED).unwrap_err();assert!(matches!(err,Error::Io(_)),"{err}");assert!(err.to_string().contains("spawn failed"),"{err}");}#[test]fn the_probe_is_the_help_flag_and_never_a_bare_invocation(){assert_eq!(GOFMT_PROBE_ARGS,["-h"]);assert!(!GOFMT_PROBE_ARGS.is_empty());}#[test]fn system_tools_locate_walks_the_candidates_in_order(){assert_eq!(SystemTools.locate(&["tokenpress-no-such-tool","gofmt"]),Some("gofmt".to_string()));assert_eq!(SystemTools.locate(&["tokenpress-no-such-tool"]),None);assert_eq!(SystemTools.locate(&GOFMT_NAMES),Some("gofmt".to_string()));}#[test]fn system_tools_run_reports_gofmts_verdict(){let scratch=Scratch::new().unwrap();let good=scratch.write("good.go",VALID).unwrap();let bad=scratch.write("bad.go","package main\nfunc main() {\n").unwrap();assert_eq!(verdict(SystemTools.run("gofmt",&GOFMT_ARGS,&good).unwrap()),"");let message=verdict(SystemTools.run("gofmt",&GOFMT_ARGS,&bad).unwrap());assert!(message.contains("expected ';', found 'EOF'"),"{message}");}#[test]fn what_gofmt_prints_to_stdout_is_not_the_verdict(){let scratch=Scratch::new().unwrap();let unformatted=scratch.write("unformatted.go",FORMATTED).unwrap();assert_eq!(verdict(SystemTools.run("gofmt",&GOFMT_ARGS,&unformatted).unwrap()),"");check_with(&SystemTools,VALID,FORMATTED).unwrap();}#[test]fn gofmt_accepts_output_it_can_still_parse(){check_with(&SystemTools,"package main\n\nimport \"fmt\"\n\nfunc main() {\n\tfmt.Println(\"hi\")\n}\n","package main\nimport \"fmt\"\nfunc main() {\nfmt.Println(\"hi\")\n}\n",).unwrap();}#[test]fn gofmt_rejects_output_it_cannot_parse(){let err=check_with(&SystemTools,VALID,"package main\nfunc main() {\n").unwrap_err();let message=err.to_string();assert!(message.contains("external check failed (gofmt -e)"),"{message}");assert!(message.contains("expected"),"{message}");}#[test]fn input_gofmt_already_rejects_does_not_fail_the_run(){check_with(&SystemTools,"\n\n// note\n\n","// note\n").unwrap();check_with(&SystemTools,"","").unwrap();}#[test]fn check_runs_the_real_gofmt(){check(VALID,FORMATTED).unwrap();let err=check(VALID,"package main\nfunc main() {\n").unwrap_err();assert!(err.to_string().contains("external check failed"),"{err}");}#[test]fn scratch_directories_are_unique_and_removed_on_drop(){let first=Scratch::new().unwrap();let second=Scratch::new().unwrap();assert_ne!(first.dir,second.dir);let path=first.write("a.go",VALID).unwrap();assert_eq!(std::fs::read_to_string(&path).unwrap(),VALID);let dir=first.dir.clone();drop(first);assert!(!dir.exists());}}