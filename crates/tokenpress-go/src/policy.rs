...
use std::ops::Range;use tokenpress_treesitter::emit::{CommentPolicy,Span,SpanKind};use tokenpress_treesitter::parser::{Node,Tree};
...
pub fn is_semantic_comment(bytes:&[u8])->bool{is_directive_comment(bytes)||bytes.starts_with(b"/*line ")||is_build_constraint(bytes)}
...
pub fn is_directive_comment(bytes:&[u8])->bool{let Some(text)=bytes.strip_prefix(b"//")else{return false;};if text.starts_with(b"line ")||text.starts_with(b"extern ")||text.starts_with(b"export "){return true;}let Some(colon)=text.iter().position(|byte|*byte==b':')else{return false;};if colon==0||colon+1>=text.len(){return false;}text[..=colon+1].iter().enumerate().all(|(index,byte)|index==colon||byte.is_ascii_lowercase()||byte.is_ascii_digit())}
...
pub fn is_promotable_directive(source:&[u8],span:Span)->bool{span.kind==SpanKind::Comment&&!starts_a_line(source,span.start)&&is_directive_comment(&source[span.range()])}
...
/// case.
pub fn build_constraint_prologue(tree:&Tree,source:&[u8])->Range<usize>{let Some(end)=package_clause_start(tree)else{return 0..0;};if source[..end].split(|byte|*byte==b'\n').any(is_build_constraint){0..end}else{0..0}}
...
pub fn imports_c(tree:&Tree,source:&[u8])->bool{node_imports_c(tree.root_node(),source)}
...
pub type GoCommentPolicy=CommentPolicy<fn(&[u8])->bool,fn(&Tree,&[u8])->Range<usize>,fn(&Tree,&[u8])->bool>;
...
pub fn comment_policy()->GoCommentPolicy{CommentPolicy::new(is_semantic_comment,build_constraint_prologue,imports_c)}
...
fn is_build_constraint(line:&[u8])->bool{let line=line.trim_ascii();let Some(after_slashes)=line.strip_prefix(b"//")else{return false;};line.starts_with(b"//go:build")||after_slashes.trim_ascii_start().starts_with(b"+build")}
/// Whether the byte at `start` is the first of its line.
///
/// Byte 0 is, by definition: there is no preceding newline because there is
/// no preceding byte.
fn starts_a_line(source:&[u8],start:usize)->bool{start==0||source[start-1]==b'\n'}
...
fn package_clause_start(tree:&Tree)->Option<usize>{let root=tree.root_node();let mut cursor=root.walk();let mut children=root.children(&mut cursor);children.find(|child|child.kind()=="package_clause").map(|clause|clause.start_byte())}
...
fn node_imports_c(node:Node,source:&[u8])->bool{let mut cursor=node.walk();let mut children=node.children(&mut cursor);if node.kind()=="import_spec"{children.any(|child|is_c_import_path(&source[child.byte_range()]))}else{children.any(|child|node_imports_c(child,source))}}
/// Whether these are the bytes of an import path naming the cgo pseudo-package.
fn is_c_import_path(bytes:&[u8])->bool{bytes==b"\"C\""||bytes==b"`C`"}#[cfg(test)]mod tests{use super::*;use crate::config::go_config;use tokenpress_treesitter::emit::{minimize,strip_comments_pinned,strip_comments_plan,strip_comments_source,};use tokenpress_treesitter::parser::parse;
/// The comments-stripped emitter as G3 will assemble it, minus the
/// column-0 pin — the engine's own end-to-end entry point.
fn stripped(source:&[u8])->Vec<u8>{strip_comments_source(&go_config(),source,&comment_policy()).unwrap()}
/// The whole composition, pin included: what G3 has to build.
fn stripped_and_pinned(source:&[u8])->Vec<u8>{let config=go_config();let tree=parse(&config,source).unwrap();let plan=strip_comments_plan(&config,&tree,source,&comment_policy());strip_comments_pinned(source,&plan,minimize(&config),|span|{is_promotable_directive(source,span)})}
/// The prologue range for a source the engine accepts.
fn prologue(source:&[u8])->Range<usize>{let config=go_config();let tree=parse(&config,source).unwrap();build_constraint_prologue(&tree,source)}
/// Whether the bail-out fires for a source the engine accepts.
fn bails_out(source:&[u8])->bool{let config=go_config();let tree=parse(&config,source).unwrap();imports_c(&tree,source)}#[test]fn the_go_toolchains_own_is_directive_table_holds(){for(text,expected)in[("abc",false),("go:inline",true),("Go:inline",false),("go:Inline",false),(":inline",false),("lint:ignore",true),("lint:1234",true),("1234:lint",true),("go: inline",false),("go:",false),("go:*",false),("go:x*",true),("export foo",true),("extern foo",true),("expert foo",false),]{let comment=format!("//{text}");assert_eq!(is_directive_comment(comment.as_bytes()),expected,"{comment}");}}#[test]fn the_directive_prefixes_need_their_trailing_space(){assert!(!is_directive_comment(b"//line"));assert!(!is_directive_comment(b"//export"));assert!(!is_directive_comment(b"//extern"));assert!(is_directive_comment(b"//line foo.go:1"));assert!(is_directive_comment(b"//line:1"));}#[test]fn only_the_first_colon_decides(){assert!(is_directive_comment(b"//go:generate echo A:B:C"));assert!(is_directive_comment(b"//a:b:c"));assert!(!is_directive_comment(b"//go generate:x"));}#[test]fn a_block_comment_is_never_a_line_directive(){assert!(!is_directive_comment(b"/*go:inline*/"));assert!(!is_directive_comment(b"/*line foo.go:1*/"));assert!(!is_directive_comment(b"/* not a comment marker at all */"));assert!(!is_directive_comment(b"go:inline"));}#[test]fn every_directive_family_is_semantic(){for comment in[&b"//go:build linux"[..],b"//go:generate stringer -type=T",b"//go:embed data.txt",b"//go:noinline",b"//go:linkname localname target",b"//line foo.go:12:3",b"//export GoCallback",b"//extern gccgo_symbol",b"//lint:ignore SA1000 reason",]{let text=String::from_utf8_lossy(comment);assert!(is_semantic_comment(comment),"{text}");}}#[test]fn the_block_line_directive_is_semantic_but_no_block_go_directive_is(){assert!(is_semantic_comment(b"/*line foo.go:12*/"));assert!(!is_semantic_comment(b"/*go:embed data.txt*/"));assert!(!is_semantic_comment(b"/*go:build ignoreme*/"));assert!(!is_semantic_comment(b"/*linefoo.go:12*/"));}#[test]fn the_legacy_build_constraint_is_semantic(){assert!(!is_directive_comment(b"// +build linux"));assert!(is_semantic_comment(b"// +build linux"));assert!(is_semantic_comment(b"//+build linux"));}#[test]fn an_ordinary_comment_is_not_semantic(){for comment in[&b"// Package main does things."[..],b"//",b"// TODO: fix this",b"/* a block comment */",b"// go:generate not-a-directive",]{let text=String::from_utf8_lossy(comment);assert!(!is_semantic_comment(comment),"{text}");}}#[test]fn an_indented_line_directive_is_promotable_and_a_column_zero_one_is_not(){let source=b"package main\n\nfunc f() {\n\t//line filename:line:col\n\tprintln(1)\n}\n";let start=source.windows(2).position(|pair|pair==b"//").expect("the comment is there");let span=Span::new(start,start+24,SpanKind::Comment);assert_eq!(&source[span.range()],b"//line filename:line:col");assert!(is_promotable_directive(source,span));let at_column_zero=Span::new(start-1,start+23,SpanKind::Comment);assert!(!is_promotable_directive(b"package main\n\nfunc f() {\n//line filename:line:col\n\tprintln(1)\n}\n",at_column_zero));}#[test]fn the_indented_line_directive_from_go_scanner_survives_at_column_one(){let source=b"package main\n\nfunc f() {\n\t//line filename:line:col\n\tprintln(1)\n}\n";assert_eq!(stripped_and_pinned(source),b"package main\nfunc f() {\n //line filename:line:col\nprintln(1)\n}\n");assert_eq!(stripped(source),b"package main\nfunc f() {\n//line filename:line:col\nprintln(1)\n}\n");}#[test]fn an_indented_go_generate_is_not_promoted(){let source=b"package main\n\nfunc main() {\n\t//go:generate echo ran\n\tprintln(1)\n}\n";assert_eq!(stripped_and_pinned(source),b"package main\nfunc main() {\n //go:generate echo ran\nprintln(1)\n}\n");}#[test]fn a_column_zero_directive_keeps_its_column(){let source=b"package main\n\n//go:generate echo ran\nfunc main() {}\n";assert_eq!(stripped_and_pinned(source),b"package main\n//go:generate echo ran\nfunc main() {}\n");}#[test]fn a_protected_span_and_an_ordinary_comment_are_never_promotable(){let source=b"package main\n\nvar s = \"//go:generate x\"\n";let start=source.windows(1).position(|byte|byte==b"\"").expect("the literal is there");assert!(!is_promotable_directive(source,Span::new(start,source.len()-1,SpanKind::Protected)));let plain=b"package main\n\nfunc f() {\n\t// just a note\n}\n";let note=plain.windows(2).position(|pair|pair==b"//").expect("the comment is there");assert!(!is_promotable_directive(plain,Span::new(note,note+14,SpanKind::Comment)));}#[test]fn a_go_build_header_is_reproduced_verbatim_including_its_blank_line(){let source=b"//go:build linux && amd64\n\npackage a\n\nfunc f() {}\n";assert_eq!(prologue(source),0..27);assert_eq!(stripped(source),b"//go:build linux && amd64\n\npackage a\nfunc f() {}\n");}#[test]fn the_legacy_constraints_blank_line_survives(){let source=b"// +build ignoreme\n\npackage a\n\nfunc f() {}\n";assert_eq!(prologue(source),0..20);assert_eq!(stripped(source),b"// +build ignoreme\n\npackage a\nfunc f() {}\n");}#[test]fn the_region_covers_the_whole_header_not_just_the_constraint(){let source=b"//go:build linux\n// +build linux\n\n// Package a does things.\npackage a\n\nfunc f() {}\n";assert_eq!(prologue(source),0..60);assert_eq!(stripped(source),&b"//go:build linux\n// +build linux\n\n// Package a does things.\npackage a\nfunc f() {}\n"[..]);}#[test]fn an_indented_constraint_still_claims_the_header(){assert_eq!(prologue(b"\t//go:build linux\n\npackage a\n"),0..19);}#[test]fn a_header_without_a_constraint_claims_nothing(){let source=b"// Package a does things.\npackage a\n\nfunc f() {}\n";assert_eq!(prologue(source),0..0);assert_eq!(stripped(source),b"package a\nfunc f() {}\n");}#[test]fn a_source_with_no_package_clause_has_an_empty_prologue(){assert_eq!(prologue(b"func f() int { return 1 }\n"),0..0);assert_eq!(prologue(b"//go:build linux\n\nfunc f() {}\n"),0..0);}#[test]fn an_empty_source_has_an_empty_prologue(){assert_eq!(prologue(b""),0..0);assert_eq!(stripped(b""),b"");}#[test]fn a_cgo_file_is_left_byte_identical(){let source=b"package main\n\n/*\n#include <stdio.h>\nstatic void hello(void) { printf(\"hi\\n\"); }\n*/\nimport \"C\"\n\nfunc main() { C.hello() }\n";assert!(bails_out(source));assert_eq!(stripped(source),source);assert_eq!(stripped_and_pinned(source),source);}#[test]fn a_grouped_import_of_c_bails_out(){let source=b"package main\n\nimport (\n\t\"fmt\"\n\t/*\n\t   #include <stdio.h>\n\t*/\n\t\"C\"\n)\n\nfunc main() { fmt.Println(\"x\"); C.hello() }\n";assert!(bails_out(source));assert_eq!(stripped(source),source);}#[test]fn a_back_quoted_import_of_c_bails_out(){assert!(bails_out(b"package main\n\nimport `C`\n\nfunc main() {}\n"));}#[test]fn a_named_import_of_c_bails_out(){assert!(bails_out(b"package main\n\nimport _ \"C\"\n\nfunc main() {}\n"));}#[test]fn a_c_that_is_not_an_import_of_c_does_not_bail_out(){let source=b"package main\n\nimport (\n\t\"fmt\"\n\t\"example.com/C\"\n\t\"Csv\"\n)\n\n// note\nvar c = \"C\"\n\nfunc main() { fmt.Println(c, C.X, Csv.Y) }\n";assert!(!bails_out(source));assert!(!stripped(source).windows(6).any(|w|w==b"// not"));}#[test]fn the_embed_directive_survives_and_stays_attached_to_its_var(){let source=b"package main\n\nimport _ \"embed\"\n\n//go:embed data.txt\nvar s string\n\nfunc main() { println(s) }\n";assert_eq!(stripped(source),&b"package main\nimport _ \"embed\"\n//go:embed data.txt\nvar s string\nfunc main() { println(s) }\n"[..]);}#[test]fn deleting_a_comment_between_the_directive_and_its_var_does_not_separate_them(){let source=b"package main\n\nimport _ \"embed\"\n\n//go:embed data.txt\n// a note\nvar s string\n";assert_eq!(stripped(source),&b"package main\nimport _ \"embed\"\n//go:embed data.txt\nvar s string\n"[..]);}#[test]fn the_policy_is_the_three_decisions_in_one_call(){let source=b"//go:build linux\n\npackage a\n\n// dropped\n//go:noinline\nfunc f() {}\n";assert_eq!(stripped(source),&b"//go:build linux\n\npackage a\n//go:noinline\nfunc f() {}\n"[..]);}}