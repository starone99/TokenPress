...
/// Go-specific choices.
#[derive(Clone,Debug,Default)]pub struct GoOptions{
...
pub strip_comments:bool,}pub struct GoFormatter{options:GoOptions,}impl GoFormatter{pub fn new(options:GoOptions)->Self{Self{options}}}impl Default for GoFormatter{fn default()->Self{Self::new(GoOptions::default())}}
...
fn comment_policy(strip_comments:bool)->policy::GoCommentPolicy{let keep_comment=if strip_comments{policy::is_semantic_comment}else{keep_every_comment as fn(&[u8])->bool};CommentPolicy::new(keep_comment,policy::build_constraint_prologue,policy::imports_c,)}impl Formatter for GoFormatter{fn language(&self)->&'static str{"go"}fn supports(&self,path:&Path)->bool{paths::supports_path(path)}
...
fn format(&self,_path:&Path,source:&str,options:&FormatOptions)->Result<FormatResult>{let bytes=source.as_bytes();let config=config::go_config();let tree=parser::parse(&config,bytes)?;let plan=emit::strip_comments_plan(&config,&tree,bytes,&comment_policy(self.options.strip_comments),);let emitted=emit::strip_comments_pinned(bytes,&plan,emit::minimize(&config),|span|{policy::is_promotable_directive(bytes,span)});let code=String::from_utf8_lossy(&emitted).into_owned();match options.verify{VerifyLevel::Reparse=>{verify::reparse(&config,code.as_bytes())?;}VerifyLevel::AstEquiv=>{verify::equivalent(&config,bytes,code.as_bytes())?;}VerifyLevel::External=>{verify::equivalent(&config,bytes,code.as_bytes())?;external::check(source,&code)?;}}let tokenizer=options.tokenizer.load()?;Ok(FormatResult{original_tokens:tokenizer.count(source),formatted_tokens:tokenizer.count(&code),code,})}}#[cfg(test)]mod tests{use super::*;use std::path::Path;use tokenpress_core::{FormatOptions,Formatter,TokenizerKind,VerifyLevel};fn fmt(source:&str)->String{fmt_with(source,GoOptions::default())}fn stripped(source:&str)->String{fmt_with(source,GoOptions{strip_comments:true,},)}fn fmt_with(source:&str,options:GoOptions)->String{GoFormatter::new(options).format(Path::new("a.go"),source,&FormatOptions::default()).unwrap().code}#[test]fn language_is_go(){assert_eq!(GoFormatter::default().language(),"go");}#[test]fn supports_the_go_paths(){let f=GoFormatter::default();for name in["a.go","main.go","internal/deep/thing_test.go"]{assert!(f.supports(Path::new(name)),"{name} should be supported");}for name in["a.rb","a.py","go.mod","go","a.GO"]{assert!(!f.supports(Path::new(name)),"{name} should be rejected");}}#[test]fn go01_minimizes_whitespace(){let source="package main\n\nimport \"fmt\"\n\nfunc main() {\n\tfmt.Println(\"hi\")\n}\n";assert_eq!(fmt(source),"package main\nimport \"fmt\"\nfunc main() {\nfmt.Println(\"hi\")\n}\n");}#[test]fn go01_collapses_gofmt_alignment_columns(){let source="package main\n\nvar (\n\ta   = 1\n\tbbb = 2\n)\n";assert_eq!(fmt(source),"package main\nvar (\na = 1\nbbb = 2\n)\n");}#[test]fn go01_normalizes_crlf(){let source="package main\r\n\r\nfunc f() {}\r\n";assert_eq!(fmt(source),"package main\nfunc f() {}\n");}#[test]fn go01_drops_the_leading_run_of_a_source_with_no_package_clause(){assert_eq!(fmt("\tfunc  f()  {}\n"),"func f() {}\n");}#[test]fn token_counts_are_reported(){let source="package main\n\nfunc main() {\n\tx := 1\n\t_ = x\n}\n";let r=GoFormatter::default().format(Path::new("a.go"),source,&FormatOptions::default()).unwrap();assert!(r.original_tokens>0);assert!(r.formatted_tokens>0);assert!(r.formatted_tokens<r.original_tokens);assert!(r.tokens_saved()>0);}#[test]fn the_path_is_not_read(){let r=GoFormatter::default().format(Path::new("notes.txt"),"package  main\n",&FormatOptions::default(),).unwrap();assert_eq!(r.code,"package main\n");}#[test]fn goo1_defaults_to_keeping_comments(){assert!(!GoOptions::default().strip_comments);}#[test]fn goo1_keeps_every_comment_by_default(){let source="package main\n\n// leading\nfunc main() {\n\tx := 1 // trailing\n\t/* inline */ _ = x\n}\n";assert_eq!(fmt(source),"package main\n// leading\nfunc main() {\nx := 1 // trailing\n/* inline */ _ = x\n}\n");}#[test]fn goo1_strips_comments_on_request(){let source="package main\n\n// leading\nfunc main() {\n\tx := 1 // trailing\n\t/* inline */ _ = x\n}\n";assert_eq!(stripped(source),"package main\nfunc main() {\nx := 1\n_ = x\n}\n");}#[test]fn a_comment_only_file_is_emptied_rather_than_refused(){let source="// only a comment\n";assert_eq!(stripped(source),"");let r=GoFormatter::new(GoOptions{strip_comments:true,}).format(Path::new("a.go"),source,&FormatOptions::default()).unwrap();assert_eq!(r.formatted_tokens,0);}#[test]fn goo1_keeps_semantic_comments_when_stripping(){let source="package main\n\n//go:generate echo hi\nfunc f() {}\n\n//line a.go:10\nfunc g() {\n\t// noise\n}\n";assert_eq!(stripped(source),"package main\n//go:generate echo hi\nfunc f() {}\n//line a.go:10\nfunc g() {\n}\n");}#[test]fn an_indented_directive_is_never_promoted_to_column_0(){let source="package main\n\nfunc main() {\n\t//go:generate echo PROMOTED\n\tx := 1\n\t_ = x\n}\n";let expected="package main\nfunc main() {\n //go:generate echo PROMOTED\nx := 1\n_ = x\n}\n";assert_eq!(fmt(source),expected);assert_eq!(stripped(source),expected);}#[test]fn a_column_0_directive_is_not_indented_by_the_pin(){let source="package main\n\nfunc f() {}\n\n//go:noinline\nfunc g() {}\n";let expected="package main\nfunc f() {}\n//go:noinline\nfunc g() {}\n";assert_eq!(fmt(source),expected);assert_eq!(stripped(source),expected);}#[test]fn the_build_constraint_prologue_survives_verbatim_at_both_settings(){let source="//go:build ignore\n// +build ignore\n\npackage main\n\nfunc main() {\n\tx  :=  1\n\t_ = x\n}\n";let expected="//go:build ignore\n// +build ignore\n\npackage main\nfunc main() {\nx := 1\n_ = x\n}\n";assert_eq!(fmt(source),expected);assert_eq!(stripped(source),expected);}#[test]fn a_header_without_a_build_constraint_is_minimized_normally(){let source="// licence\n// notes\n\npackage main\n\nfunc f() {}\n";assert_eq!(fmt(source),"// licence\n// notes\npackage main\nfunc f() {}\n");assert_eq!(stripped(source),"package main\nfunc f() {}\n");}#[test]fn a_comment_byte_adjacent_to_a_literal_takes_only_itself(){let source="package main\n\nimport \"fmt\"\n\nfunc main() {\n\tfmt.Println(/*c*/\"lit\")\n}\n";assert_eq!(stripped(source),"package main\nimport \"fmt\"\nfunc main() {\nfmt.Println( \"lit\")\n}\n");let after="package main\n\nimport \"fmt\"\n\nfunc main() {\n\tfmt.Println(\"lit\"/*c*/)\n}\n";assert_eq!(stripped(after),"package main\nimport \"fmt\"\nfunc main() {\nfmt.Println(\"lit\" )\n}\n");}#[test]fn cgo_files_are_left_byte_identical_at_both_settings(){let source="package main\n\n/*\n#include <stdio.h>\n*/\nimport \"C\"\n\nfunc main() {\n\tC.puts(C.CString(\"hi\"))\n}\n";assert_eq!(fmt(source),source);assert_eq!(stripped(source),source);}#[test]fn cgo_files_report_no_savings(){let source="package main\n\n/*\n#include <stdio.h>\n*/\nimport \"C\"\n\nfunc main() {\n\tC.puts(C.CString(\"hi\"))\n}\n";let r=GoFormatter::default().format(Path::new("a.go"),source,&FormatOptions::default()).unwrap();assert_eq!(r.original_tokens,r.formatted_tokens);assert_eq!(r.tokens_saved(),0);}#[test]fn reparse_only_level_also_passes(){let opts=FormatOptions{verify:VerifyLevel::Reparse,..FormatOptions::default()};let r=GoFormatter::default().format(Path::new("a.go"),"package  main\n",&opts).unwrap();assert_eq!(r.code,"package main\n");}#[test]fn ast_equiv_is_the_default_level(){assert_eq!(FormatOptions::default().verify,VerifyLevel::AstEquiv);let opts=FormatOptions{verify:VerifyLevel::AstEquiv,..FormatOptions::default()};let r=GoFormatter::default().format(Path::new("a.go"),"package  main\n",&opts).unwrap();assert_eq!(r.code,"package main\n");}fn external()->FormatOptions{FormatOptions{verify:VerifyLevel::External,..FormatOptions::default()}}#[test]fn external_level_runs_gofmt_on_top_of_the_built_in_check(){let r=GoFormatter::default().format(Path::new("a.go"),"package  main\n\nfunc  main()  {}\n",&external(),).unwrap();assert_eq!(r.code,"package main\nfunc main() {}\n");}#[test]fn external_level_does_not_blame_input_gofmt_already_rejects(){let r=GoFormatter::default().format(Path::new("a.go"),"\n\n// note\n\n",&external()).unwrap();assert_eq!(r.code,"// note\n");}#[test]fn parse_errors_are_reported(){let err=GoFormatter::default().format(Path::new("broken.go"),"package main\n\nfunc main() {\n",&FormatOptions::default(),).unwrap_err();assert!(matches!(err,Error::Parse(_)),"{err}");assert!(err.to_string().starts_with("parse error: syntax error at byte "),"{err}");}#[test]fn formatting_is_idempotent(){let sources=["package main\n\nimport \"fmt\"\n\nfunc main() {\n\tfmt.Println(\"hi\")\n}\n","//go:build ignore\n// +build ignore\n\npackage main\n\nfunc main() {}\n","package main\n\nfunc main() {\n\t//go:generate echo PROMOTED\n\tx := 1\n\t_ = x\n}\n","package main\n\n/*\n#include <stdio.h>\n*/\nimport \"C\"\n\nfunc main() {}\n","package main\n\nvar s = `raw\n  string\n`\n\n// note\nfunc f() {}\n",];for source in sources{let once=fmt(source);assert_eq!(fmt(&once),once,"not idempotent for {source:?}");let once_stripped=stripped(source);assert_eq!(stripped(&once_stripped),once_stripped,"not idempotent when stripping for {source:?}");}}#[test]fn tokenizer_choice_is_respected(){let opts=FormatOptions{tokenizer:TokenizerKind::Cl100kBase,..FormatOptions::default()};let r=GoFormatter::default().format(Path::new("a.go"),"package  main\n",&opts).unwrap();assert!(r.original_tokens>0);}}