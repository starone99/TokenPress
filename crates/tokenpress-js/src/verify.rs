...
use crate::emit;
use crate::parser::{self, Arena, Program};
use tokenpress_core::{Error, Result};

/// Weakest level: the output must parse at all, as the dialect implied by
/// `path`.
pub fn reparse(path: &Path, code: &str) -> Result<()> {
    let allocator = Arena::default();
    parse_output(&allocator, path, code)?;
    Ok(())
}

/// Full check: the output must parse and must be canonically identical to
/// `original`.
pub fn equivalent(original: &Program<'_>, path: &Path, code: &str) -> Result<()> {
    let allocator = Arena::default();
    let reparsed = parse_output(&allocator, path, code)?;
    if emit::canonical(original) != emit::canonical(&reparsed) {
...
fn parse_output<'a>(allocator: &'a Arena, path: &Path, code: &'a str) -> Result<Program<'a>> {
    parser::parse(allocator, path, code)
...
mod tests {
    ...
    fn check(name: &str, source: &str, code: &str) -> Result<()> {
        let allocator = Arena::default();
        let path = Path::new(name);
        let program = parser::parse(&allocator, path, source).unwrap();
        equivalent(&program, path, code)
    }

    #[test]
    fn reparse_accepts_valid_and_rejects_invalid_output() {
        assert!(reparse(Path::new("a.js"), "const a=1;").is_ok());
        let err = reparse(Path::new("a.js"), "function (").unwrap_err();
        assert!(err.to_string().contains("failed to re-parse"), "{err}");
    }

    #[test]
    fn reparse_rejects_recoverable_errors_too() {
        // `parser::parse` rejects on diagnostics as well as on `panicked`,
        // and verification inherits both checks.
        let err = reparse(Path::new("a.js"), "return 1;").unwrap_err();
        assert!(err.to_string().contains("failed to re-parse"), "{err}");
    }

    #[test]
    fn formatting_only_differences_are_equivalent() {
        assert!(check(
            "a.js",
            "function add( a , b ) {\n    return a + b;\n}\n",
    ...
    #[test]
    fn semantic_change_is_rejected() {
        let err = check("a.js", "const a = 1 + 2;\n", "const a=1-2;").unwrap_err();
        assert!(err.to_string().contains("differs from input"), "{err}");
    }

    #[test]
    fn dropped_typescript_annotation_is_rejected() {
        let err = check("a.ts", "const a: number = 1;\n", "const a=1;").unwrap_err();
        assert!(err.to_string().contains("differs from input"), "{err}");
    }

    #[test]
    fn unparsable_output_is_rejected_by_the_equivalence_check() {
        let err = check("a.js", "const a = 1;\n", "function (").unwrap_err();
        assert!(err.to_string().contains("failed to re-parse"), "{err}");
    }

    #[test]
    fn comment_loss_is_not_caught_by_the_verifier() {
        // Documented hole: canonicalization erases comments, so a lost
